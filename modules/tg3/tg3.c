...
#include "tg3_pci_ids.h"
#include "tg3_compat.h"
#include "tg3.h"

#define DRV_MODULE_NAME		"tg3"
#define PFX DRV_MODULE_NAME	": "
#define DRV_MODULE_VERSION	"2.3_ps4"
...
#define MAX_tg3_irq_poll 16

static struct {
	struct net_device *netdev;
} tg3_irq_poll[MAX_tg3_irq_poll];

static int tg3_irq_poll_last = 0;

static void
tg3_add_poll(struct net_device *netdev)
{
	int i;
	
	if (tg3_irq_poll_last == MAX_tg3_irq_poll - 1)
		return;

	for (i = 0; i < tg3_irq_poll_last; i++) {
		if (tg3_irq_poll[i].netdev == netdev)
			break;
	}
	tg3_irq_poll[i].netdev = netdev;

	if (i == tg3_irq_poll_last) 
		tg3_irq_poll_last++;
}

static void
tg3_del_poll(struct net_device *netdev)
{
	int i;
	for (i = 0; i < tg3_irq_poll_last; i++) {
		if (tg3_irq_poll[i].netdev == netdev)
	...
	if ((i < tg3_irq_poll_last) &&
	    (tg3_irq_poll[i].netdev == netdev)) {
		tg3_irq_poll_last--;
		tg3_irq_poll[i].netdev =
			tg3_irq_poll[tg3_irq_poll_last].netdev;
	}
}
...
p4ether_netif_rx_t *tg3_netif_rx = netif_receive_skb;
...
static int tg3_rx(struct tg3 *tp, int budget)
{
	...
	while (sw_idx != hw_idx && budget > 0) {
		...
		if (tp->vlgrp != NULL &&
			...
			tg3_netif_rx(skb); /* ParaStation Hook for fast-receive */
...
static int tg3_open(struct net_device *dev)
{
	...
	tg3_add_poll(dev); /* ParaStation add polling. */
...
static int tg3_close(struct net_device *dev)
{
	...
	tg3_del_poll(dev); /* ParaStation del polling */
...
tg3_polling(void)
{
	...
	for (i = 0; i < tg3_irq_poll_last; i++) {
		struct net_device *netdev = tg3_irq_poll[i].netdev;

		/* tg3_interrupt dont use irq or regs and
		   are surrouned by a lock. */
		tg3_interrupt(0, netdev, NULL);
	}
}

#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,0)
EXPORT_SYMBOL_NOVERS(tg3_polling);
EXPORT_SYMBOL_NOVERS(tg3_netif_rx);
#else
EXPORT_SYMBOL(tg3_polling);
EXPORT_SYMBOL(tg3_netif_rx);
#endif
