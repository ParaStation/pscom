...
void test_ufd_init_set_and_cleanup(void **state)
{
    ...
    int old_idx = (&precon->ufd_info)->pollfd_idx;
    ufd_event_set(ufd, &precon->ufd_info, POLLOUT);

    /* check that event has been set and index hasn't changed */
    assert_true(pollfd->events == (POLLIN | POLLOUT));
    assert_true((&precon->ufd_info)->pollfd_idx == old_idx);

    /* clear each of both events and check it */
    ufd_event_clr(ufd, &precon->ufd_info, POLLIN);
    ufd_event_clr(ufd, &precon->ufd_info, POLLOUT);
    assert_true(pollfd->events == 0);

    /* check also that pollfd/ufd_info has been removed */
    assert_true(ufd->n_ufd_pollfd == 0);
    assert_true((&precon->ufd_info)->pollfd_idx == -1);
    pollfd = ufd_get_pollfd(ufd, &precon->ufd_info);
    assert_true(pollfd == NULL);
...
void test_ufd_init_set_and_cleanup_threaded(void **state)
{
    ...
    int idx = (&precon->ufd_info)->pollfd_idx;
    assert_true(idx >= 0);
    ...
    idx = (&precon->ufd_info)->pollfd_idx;
    assert_true(idx == -1);
...
void test_ufd_do_not_write_when_con_refused(void **state)
{
    ...
    ufd_event_set(ufd, &precon->ufd_info, POLLIN | POLLOUT);
...
void test_ufd_do_not_write_when_pollfd_is_cleared(void **state)
{
    ...
    assert_true(ufd->ufd_pollfd_info[0] == &precon->ufd_info);
...
void test_ufd_do_not_read_when_array_is_updated(void **state)
{
    ...
    precon->ufd_info.can_read = &remove_global_pollfd_info_from_ufd_array;
    ...
    precon->ufd_info.can_write = &add_global_pollfd_info_to_ufd_array;
...
void test_ufd_do_not_write_when_array_is_updated(void **state)
{
    ...
    ufd_event_set(&pscom.ufd, &precon->ufd_info, POLLIN | POLLOUT);
    /* check that it has been added at the second position of the array */
    assert_true(pscom.ufd.ufd_pollfd_info[1] == &precon->ufd_info);
