...
int backup_env_vars(void **state)
{
    ...
    env_vars_backup.backup_values = (char **)malloc(env_vars_backup.count *
                                                    sizeof(char *));

    /* backup and unset all environment variables in env_vars_from_pscom */
    for (size_t i = 0; i < env_vars_backup.count; ++i) {
        env_vars_backup.backup_values[i] = getenv(env_vars_from_pscom[i]);
...
int restore_env_vars(void **state)
{
    ...
    for (size_t i = 0; i < env_vars_backup->count; ++i) {
        if (env_vars_backup->backup_values[i]) {
            setenv(env_vars_from_pscom[i], env_vars_backup->backup_values[i], 1);
        } else {
            unsetenv(env_vars_from_pscom[i]);
        }
    }
