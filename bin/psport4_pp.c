...
void parse_opt(int argc, char **argv)
{
    ...
    struct poptOption optionsTable[] = {
        ...
        {"sport", 'p', POPT_ARG_INT, &arg_sport, 0, "connect to port", "port"},
        {"loops", 'l', POPT_ARG_INT, &arg_loops, 0, "pp loops", "count"},

        {"maxsize", 0, POPT_ARG_INT, &arg_maxmsize, 0, "maximal messagesize",
         "size"},
        {"xheader", 0, POPT_ARG_INT, &arg_xheader, 0, "xheader size", "size"},
...
static void run_pp_server(PSP_PortH_t porth)
{
    ...
    req = PSP_IReceive(porth, buf, arg_maxmsize, &header.head_psp, MAX_XHEADER,
                       NULL, 0);
    PSP_Wait(porth, req);
    req = PSP_ISend(porth, buf, header.head_psp.datalen, &header.head_psp,
                    header.head_psp.xheaderlen, header.head_psp.addr.from, 0);
    PSP_Wait(porth, req);
    PSP_StopListen(porth);
    while (1) {
        req = PSP_IReceive(porth, buf, arg_maxmsize, &header.head_psp,
                           MAX_XHEADER, NULL, 0);
        PSP_Wait(porth, req);
        req = PSP_ISend(porth, buf, header.head_psp.datalen, &header.head_psp,
                        header.head_psp.xheaderlen, header.head_psp.addr.from,
                        0);
        PSP_Wait(porth, req);
    }
    free(buf);
...
static void run_pp_server_verbose(PSP_PortH_t porth)
{
    void *buf = malloc(arg_maxmsize);
    PSP_RequestH_t req;
    int i;
    ...
    for (i = 0; i < MAX_XHEADER; i++) { header.xhead[i] = (char)(i + 0xe1); }
    req = PSP_IReceive(porth, buf, arg_maxmsize, &header.head_psp, MAX_XHEADER,
                       NULL, 0);
    PSP_Wait(porth, req);
...
static int run_pp_c(PSP_PortH_t porth, int conid, int msize, int xsize,
                    int loops)
{
    int cnt;
    ...
    for (cnt = 0; cnt < loops; cnt++) {
        PSP_ISend(porth, sbuf, msize, &sheader.head_psp, xsize, conid, 0);

        //	printf("SEND %d data :%s\n", msize,
        //	       dumpstr(sbuf, MIN(msize, 16)));
        rreq = PSP_IReceive(porth, rbuf, msize, &rheader.head_psp, xsize, NULL,
                            0);
...
void do_pp_client(PSP_PortH_t porth, int conid)
{
    unsigned long t1, t2;
    double time;
    double throuput;
    ...
    for (ms = 1.4142135; ms < arg_maxmsize; ms = ms * 1.4142135) {
        ...
        t1       = getusec();
        res      = run_pp_c(porth, conid, msgsize, arg_xheader, arg_loops);
        t2       = getusec();
...
int main(int argc, char **argv)
{
    ...
    if (arg_server) {
        ...
        printf("%s -c %d -p %d --loops %d -v %d\n", argv[0], PSP_GetNodeID(),
               PSP_GetPortNo(porth), arg_loops, arg_verbose);
        if (arg_verbose) {
