...
static void parse_opt(int argc, char **argv)
{
    ...
    struct poptOption optionsTable[] = {
        ...
        {"post", 0, POPT_ARG_NONE, &arg_post_bcast, 0,
...
static void run_pp(pscom_socket_t *socket, pscom_group_t *group, unsigned msize,
                   unsigned xsize, unsigned loops)
{
    unsigned cnt;
    ...
    if (arg_verbose) {
        for (cnt = 0; cnt < xsize; cnt++) { xbuf[cnt] = (char)(cnt + 1); }
    }
    ...
    if (!arg_post_bcast) {
        ...
        pscom_request_t *req = pscom_request_create(sizeof(req->xheader.bcast) +
                                                        xsize,
                                                    0);
...
static void do_pp(pscom_connection_t *con_server,
                  pscom_connection_t *con_client,
                  unsigned my_rank /* 0 == server, 1 == client */)
{
    ...
    pscom_connection_t *cons[2] = {con_server, con_client}; // server (me) as
                                                            // rank 0
...
int main(int argc, char **argv)
{
    ...
    if (!arg_client) { // server
        ...
        if (arg_xheader != 10) { printf(" --xheader=%u", arg_xheader); }
        if (arg_post_bcast) { printf(" --post"); }
        printf("\n");
        fflush(stdout);
