...
void parse_opt(int argc, char **argv)
{
    ...
    struct poptOption optionsTable[] = {
        ...
        {"cnt", 'c', POPT_ARG_INT, &arg_cnt, 0, "#clients\t(default 1)",
         "count"},
...
int assign_clients(PSP_PortH_t porth, int cnt)
{
    ...
    for (i = 0; i < cnt; i++) {
        ...
        req = PSP_IReceive(porth, NULL /*buf*/, 0 /*buflen*/, &header.head_psp,
                           sizeof(header) -
                               sizeof(header.head_psp) /* xheaderlen */,
                           NULL /* cb */, 0 /*cb_param*/);
        ...
        if (arg_verbose) {
            fprintf(stderr, "New client from node %u.%u.%u.%u:%d\n",
                    // header.head_psp.addr.from,
                    HIPQUAD(header.info.node), header.info.port);
        }
...
PSP_PortH_t start_client(int *forward_id)
{
    ...
    if (header.info.node != -1) {
        *forward_id = PSP_Connect(porth, header.info.node, header.info.port);
        ...
        if (arg_verbose) {
            fprintf(stderr, "Forward data to client %u.%u.%u.%u:%d)\n",
                    // header.head_psp.addr.from,
                    HIPQUAD(header.info.node), header.info.port);
        }
    } else {
...
int main(int argc, char **argv)
{
    ...
    if (arg_verbose > 1) { set_timer(1, 1, timer); }
