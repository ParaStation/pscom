 ...
 */
/**
 * pscom_test_fault.c: Testing fault handling of connections
 */
...
static void parse_opt(int argc, char **argv)
{
    ...
    struct poptOption optionsTable[] = {
        {"listen", 'l', POPT_ARGFLAG_SHOW_DEFAULT | POPT_ARG_INT, &arg_lport, 0,
         "run as server and listen on", "port"},


        {"client", 'c', POPT_ARGFLAG_OR | POPT_ARG_VAL, &arg_client, 1,
         "run as client", NULL},
        ...
        {"histo", 'i', POPT_ARGFLAG_OR | POPT_ARG_VAL, &arg_histo, 1,
         "Measure each ping pong", NULL},
...
static void print_faulty_req(pscom_request_t *req, const char *msg)
{
    printf("%s : request failed. State: %s\n", msg,
           pscom_req_state_str(req->state));
    pscom_dump_connection(stdout, req->connection);
}
...
static int run_pp_c(pscom_connection_t *con, int msize, int xsize, int loops)
{
    int cnt;
...
static int run_pp_c_histo(pscom_connection_t *con, int msize, int xsize,
                          int loops)
{
    int cnt;
    void *sbuf          = malloc(msize);
