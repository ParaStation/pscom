...
static void print_usage(void)
{
    ...
    printf("    -d, --documentation Display documentation to the environment "
           "configuration (requires -c)\n");
...
static void parse_opt(int argc, char **argv)
{
    ...
    while (1) {
        ...
        switch (c) {
        case 0: break;
        case 'c': opts.config = 1; break;
        case 'd': opts.documentation = 1; break;
        case 'v': opts.version = 1; break;
        case 'x': opts.expert = 1; break;
        case '?': break;
        case 'h':
...
int main(int argc, char **argv)
{
    ...
    if (opts.config) {
        ...
        if (opts.documentation) { print_flags |= PSCOM_ENV_PRINT_DOC; }

        /* hidden config is only shown if the config is printed */
        if (opts.expert) { print_flags |= PSCOM_ENV_PRINT_HIDDEN; }
    }

    if (opts.version) { setenv("PSP_DEBUG_VERSION", "1", 1); }
