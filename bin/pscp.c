 ...
 *		Norbert Eicker <hauke@par-tec.de>
 */
...
int assign_clients(PSP_PortH_t porth, int cnt)
{
	...
	if (arg_verbose > 1) {
	    ...
	    fprintf(stderr, "New client from node %s:%d\n",
		    inet_ntoa(addr), header.info.port);
	}
...
void ps_send_flush(ps_send_info_t *info)
{
	...
	PSP_Header_t	head;
	PSP_RequestH_t req;
	req = PSP_IReceiveFrom(info->porth, NULL, 0, &head, 0,
			       NULL, 0, info->dest);
...
void ps_send(ps_send_info_t *info, char *buf, unsigned int len)
{
    ...
    if (info->dest != -1) {
	info->in_use = 1;
	info->req = PSP_ISend(info->porth, buf, len, &info->head, 0,
			      info->dest, 0);
	info->tokens --;
    }
    stat_bytes_tx += len;
...
unsigned int ps_recv(ps_recv_info_t *info, char *buf, unsigned int len)
{
    ...
    req = PSP_IReceiveFrom(info->porth, buf, len, &head, 0,
			   NULL, 0, info->src);
    PSP_Wait(info->porth, req);
    rlen = head.datalen;
