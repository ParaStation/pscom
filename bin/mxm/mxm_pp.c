...
// PSMXM_MSGLEN(len) is a round up of len to the next multiple of 64 bytes (cacheline)
...
static
void mxm_rc_check(int rc, char *msg)
{
	if (rc == MXM_OK) return;
...
static
void init_bufs(struct mxm_pp_context *ctx)
{
	int rc;
...
static
void init_reqs(struct mxm_pp_context *ctx)
{
