...
static int init_ctx(struct mxm_pp_context *ctx)
{
    ...
    error = mxm_ep_get_address(ctx->mxm_ep, ctx->mxm_ep_addr, &mxm_ep_addr_len);
    if (error != MXM_OK) {
...
#else
    error = mxm_mem_map(ctx->mxm_mxmh, &ctx->mem_access_buf,
                        &ctx->mem_access_buf_size, 0, NULL, 0);
    if (error != MXM_OK) {
    ...
    error = mxm_mem_get_key(ctx->mxm_mxmh, ctx->mem_access_buf,
                            &ctx->mem_access_buf_mkey);
    if (error != MXM_OK) {
...
{
    mxm_mem_unmap(ctx->mxm_mxmh, ctx->mem_access_buf, ctx->mem_access_buf_size,
                  0);
