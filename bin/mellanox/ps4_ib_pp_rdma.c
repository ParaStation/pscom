...
/* I got the error
   THH(4): THHUL_qpm_post_send_req: Send queue is full (128 requests
   outstanding). if SEND_NOTIFICATION is disabled */
...
#define PSIB_DATA(buf, len) ((char *)(&(buf)->tail) - len)
...
static void psib_err_rc(char *str, VAPI_ret_t rc)
{
...
void print_hca_cap(VAPI_hca_vendor_t *hca_vendor_p, VAPI_hca_cap_t *hca_cap_p)
{
    ...
    printf("\tnode_guid = %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\n",
           hca_cap_p->node_guid[0], hca_cap_p->node_guid[1],
           hca_cap_p->node_guid[2], hca_cap_p->node_guid[3],
           hca_cap_p->node_guid[4], hca_cap_p->node_guid[5],
           hca_cap_p->node_guid[6], hca_cap_p->node_guid[7]);
    printf("\tlocal_ca_ack_delay (Log2 4.096usec Max. RX to ACK or NAK delay) "
           "= %d\n",
    ...
    printf("\tmax_ee_init_rd_atom = %d\n", hca_cap_p->max_ee_init_rd_atom);
    {
        ...
        case VAPI_ATOMIC_CAP_NONE: strcpy(s, "VAPI_ATOMIC_CAP_NONE"); break;
        case VAPI_ATOMIC_CAP_HCA: strcpy(s, "VAPI_ATOMIC_CAP_HCA"); break;
        case VAPI_ATOMIC_CAP_GLOB: strcpy(s, "VAPI_ATOMIC_CAP_GLOB"); break;
        default: strcpy(s, "invalid capability");
        }
        printf("\tatomic_cap = %s\n", s);
    }
...
static int psib_get_hca_name(OUT VAPI_hca_id_t *hca_id,
                             IN const char *in_hca_name)
{
    ...
    while ((rc = EVAPI_list_hcas(1, &num_of_hcas, &inst_hca_id)) ==
           VAPI_EAGAIN) {};
    if (rc != VAPI_OK) { goto err_EVAPI_list_hcas; }

    switch (num_of_hcas) {
    case 0: psib_err("There are no HCAs installed in your system!"); return -1;
    case 1:
        strcpy(*hca_id, inst_hca_id);
        if (psib_debug) { printf("Using HCA: %s\n", *hca_id); }
        break;
    default:
        strcpy(*hca_id, inst_hca_id);
        if (psib_debug) {
            ...
            printf("The following HCAs are installed in your system. "
                   "Please use <-h hcaid> to specify one (not "
                   "implemented!).\n");
...
static int psib_open_hca(IN char *hca_name, OUT VAPI_hca_hndl_t *hca_hndl)
{
...
static int psib_open_cq(IN VAPI_hca_hndl_t hca_hndl, IN unsigned int cqe_num,
                        OUT VAPI_cq_hndl_t *cq_hndl)
{
...
static int psib_open_pd(IN VAPI_hca_hndl_t hca_hndl, OUT VAPI_pd_hndl_t *pd_hndl)
{
    VAPI_ret_t rc;
...
static int psib_open_hca_port(IN VAPI_hca_hndl_t hca_hndl, IN int hca_port_idx,
                              ...
                              OUT IB_port_t *hca_port_idx_out)
{
    ...
    if (hca_port_idx > 0) {
        ...
        while (1) {
            rc = VAPI_query_hca_port_prop(hca_hndl, port, hca_port);
            if (rc == VAPI_OK) {
                if (hca_port->state == PORT_ACTIVE) { break; }
            }
            if (++port > 32) { goto err_alllinkdown; }
        }
    }

    if (psib_debug) { printf("Using HCA Port: %d\n", port); }
...
static void psib_vapi_free(IN hca_info_t *hca_info, IN mem_info_t *mem_info)
{
    VAPI_deregister_mr(hca_info->hca_hndl, mem_info->mr_hndl);
}

static int psib_vapi_alloc(IN hca_info_t *hca_info, IN int size,
                           IN VAPI_mrw_acl_t perm, OUT mem_info_t *mem_info)
/*
...
*/
{
    ...
    /* Region for buffers */
    ptr = malloc(size); /* ToDo: Allocato to page boundary */
    ...
    rc = VAPI_register_mr(hca_info->hca_hndl, &mrw, &mem_info->mr_hndl,
                          &mem_info->rep_mrw);
    if (rc != VAPI_SUCCESS) { goto err_VAPI_register_mr; }

    if (psib_debug > 1) {
        printf("lkey: 0x%08x rkey: 0x%08x ptr: %p start: 0x%016lx\n",
               mem_info->rep_mrw.l_key, mem_info->rep_mrw.r_key, ptr,
               (long)mem_info->rep_mrw.start);
    }
...
static void psib_cleanup_con(hca_info_t *hca_info, psib_con_info_t *con_info)
{
 ...
 *  move_to_rtr
 */
static int move_to_rtr(IN VAPI_hca_hndl_t hca_hndl, IN VAPI_qp_hndl_t qp_hndl,
 ...
 *  move_to_rts
 */
static int move_to_rts(IN VAPI_hca_hndl_t hca_hndl, IN VAPI_qp_hndl_t qp_hndl)
{
    VAPI_qp_attr_t qp_attr;
    VAPI_qp_attr_mask_t qp_attr_mask;
    VAPI_qp_cap_t qp_cap;
...
static int psib_init_con(hca_info_t *hca_info, port_info_t *port_info,
                         psib_con_info_t *con_info)
{
    ...
    qp_init_attr.cap.max_sg_size_sq = 4;      /* Max scatter/gather descriptor
                                                 entries on the SQ */
    qp_init_attr.cap.max_sg_size_rq = 4;      /* Max scatter/gather descriptor
                                                 entries on the RQ */
    qp_init_attr.cap.max_inline_data_sq = 16; /* Max bytes in inline data on the
                                                 SQ */
    /* max_inline_data_sq is currently valid only for VAPI_query_qp (ignored for
     * VAPI_create_qp) */
    /* In order to enlarge the max_inline_data_sq capability, enlarge the
     * max_sg_size_sq parameter */
    ...
    /* create the QP */
    rc = VAPI_create_qp(hca_info->hca_hndl, &qp_init_attr, &con_info->qp_hndl,
                        &con_info->qp_prop);
    if (rc != VAPI_SUCCESS) { goto err_VAPI_create_qp; }

    if (psib_debug > 1) { print_qp_props(&con_info->qp_prop); }
    ...
    qp_attr.port = port_info->hca_port_idx;
    ...
    rc = VAPI_modify_qp(hca_info->hca_hndl, con_info->qp_hndl, &qp_attr,
                        &qp_attr_mask, &qp_cap);
    if (rc != VAPI_SUCCESS) { goto err_VAPI_modify_qp; }
    ...
    if (psib_vapi_alloc(hca_info, IB_MTU * SIZE_SR_QUEUE, 0,
                        &con_info->send_bufs)) {
        goto err_alloc;
    }
    con_info->send_pos = 0;
    ...
    if (psib_vapi_alloc(hca_info, IB_MTU * SIZE_SR_QUEUE,
                        VAPI_EN_REMOTE_WRITE | VAPI_EN_LOCAL_WRITE,
                        &con_info->recv_bufs)) {
        goto err_alloc;
    }
...
static int psib_connect_con(psib_con_info_t *con_info,
                            IB_lid_t remote_lid /* remote peer's LID */,
                            IB_wqpn_t remote_qpn /* remote peer's QPN */,
                            void *remote_ptr, VAPI_rkey_t remote_rkey)
{
    int i;
    ...
    if (move_to_rtr(con_info->hca_hndl, con_info->qp_hndl, remote_lid,
                    remote_qpn)) {
    ...
    if (move_to_rts(con_info->hca_hndl, con_info->qp_hndl)) {
...
static void psib_cleanup_hca(hca_info_t *hca_info)
{
    if (hca_info->pd_hndl) {
    ...
    if (hca_info->hca_hndl) { EVAPI_release_hca_hndl(hca_info->hca_hndl); }
}

static int psib_init_hca(hca_info_t *hca_info)
{
    VAPI_hca_hndl_t hca_hndl;
...
static int psib_init_port(hca_info_t *hca_info, port_info_t *port_info)
{
    if (psib_open_hca_port(hca_info->hca_hndl, -1, &port_info->hca_port,
                           &port_info->hca_port_idx)) {
        goto err_port;
    }
...
static int psib_send(psib_con_info_t *con_info, void *buf, int size)
{
    int len;
    psib_msg_t *msg;
    ...
    */
    ...
    msg->tail.token   = con_info->n_tosend_toks;
    msg->tail.payload = len;
    msg->tail.magic   = 0x1;

    memcpy(PSIB_DATA(msg, len), buf, len);

    sg_lst.addr = (MT_virt_addr_t)PSIB_DATA(msg, len);
    sg_lst.len  = len + sizeof(psib_msgheader_t);
    sg_lst.lkey = con_info->send_bufs.rep_mrw.l_key;

    sr_desc.id     = (VAPI_wr_id_t)(unsigned long)con_info; /* User defined work
                                                               request ID */
    sr_desc.opcode = VAPI_RDMA_WRITE; // use VAPI_SEND_WITH_IMM to send also
                                      // imm_data
    //    sr_desc.opcode = VAPI_SEND_WITH_IMM; // use VAPI_SEND_WITH_IMM to send
    //    also imm_data
    /* VAPI_SEND_WITH_IMM on lyra about +0.06 us */
    sr_desc.comp_type  = ENABLE_SEND_NOTIFICATION
                             ? VAPI_SIGNALED
                             : VAPI_UNSIGNALED; /* no cq entry, if unsignaled */
    ...
    sr_desc.remote_addr = (MT_virt_addr_t)PSIB_DATA(
        (((psib_msg_t *)con_info->remote_ptr) + con_info->send_pos), len);

    sr_desc.r_key = con_info->remote_rkey;

    //    printf("Send to 0x%lx 0x%x magic %08x\n", sr_desc.remote_addr,
    //    sr_desc.r_key,
    //	   msg->tail.magic);

    rc = VAPI_post_sr(con_info->hca_hndl, con_info->qp_hndl, &sr_desc);
    //    rc = EVAPI_post_inline_sr(con_info->hca_hndl, con_info->qp_hndl,
    //    &sr_desc);
...
static int psib_recvlook(psib_con_info_t *con_info, void **buf)
{
    ...
    while (con_info->n_recv_toks > 0) {
        psib_msg_t *msg = ((psib_msg_t *)con_info->recv_bufs.ptr) +
                          con_info->recv_pos;
        int len = msg->tail.payload;

        *buf = PSIB_DATA(msg, len);
        if (len) { return len; }
...
static VAPI_ret_t psib_check_cq(hca_info_t *hca_info)
{
    ...
    if (rc == VAPI_OK) {
        if (comp_desc.opcode == VAPI_CQE_SQ_RDMA_WRITE) {
            /* RDMA write done */
            psib_con_info_t *con = (psib_con_info_t *)(unsigned long)
                                       comp_desc.id;
            if (comp_desc.status == VAPI_SUCCESS) {
                // printf("RDMA write done... recv: %d tosend: %d send: %d\n",
                //        con->n_recv_toks, con->n_tosend_toks,
                //        con->n_send_toks);
            } else {
                if (psib_debug > 0) {
                    printf("Failed RDMA write request (status %d). Connection "
                           "broken!\n",
                           comp_desc.status);
                }
                con->con_broken = 1;
            ...
            if (comp_desc.status == VAPI_SUCCESS) {
                ...
                //		if ((con->n_recv_toks == 0) &&
                //(msg->header.payload == 0)) {
                ...
                if (psib_debug > 0) {
                    printf("Failed receive request (status %d). Connection "
                           "broken!\n",
                           comp_desc.status);
                }
                con->con_broken = 1;
            ...
            psib_con_info_t *con = (psib_con_info_t *)(unsigned long)
                                       comp_desc.id;
            if (comp_desc.status == VAPI_SUCCESS) {
                //		printf("Send done... recv: %d tosend: %d send:
                //%d\n", 		       con->n_recv_toks,
                // con->n_tosend_toks, con->n_send_toks);
            } else {
                if (psib_debug > 0) {
                    printf("Failed send request (status %d). Connection "
                           "broken!\n",
                           comp_desc.status);
                }
                con->con_broken = 1;
...
static int psib_poll(hca_info_t *hca_info, int blocking)
{
    VAPI_ret_t rc;
    ...
    if (psib_debug && (rc != VAPI_CQ_EMPTY) && (rc != VAPI_OK)) {
...
static void run_pp_server(hca_info_t *hca, psib_con_info_t *con)
{
    void *rbuf;
    int ret;
    int len;
...
static int run_pp(hca_info_t *hca, psib_con_info_t *con, int loops, int msize)
{
    int cnt;
...
void do_pp(void)
{
    ...
    printf("Local LID / QP / rkey /ptr:  0x%04x 0x%06x 0x%06x %p \n", con.lid,
           con.qp_prop.qp_num, con.recv_bufs.rep_mrw.r_key, con.recv_bufs.ptr);
    {
        int rlid, rqpn;
        unsigned long rkey, ptr;
        while (1) {
            printf("Remote LID and QP rkey ptr?\n");
            fflush(stdout);
            if (scanf("%x %x %lx %lx", &rlid, &rqpn, &rkey, &ptr) == 4) {
                break;
            }
            printf("wrong format...\n");
        }
    ...
    if (psib_connect_con(&con, remote_lid, remote_qpn, remote_ptr,
                         remote_rkey)) {
