...
static
int psib_send(psib_con_info_t *con_info, void *buf, int size)
{
    int len;
    psib_msg_t *msg;
...
//    sr_desc.opcode = VAPI_SEND_WITH_IMM; // use VAPI_SEND_WITH_IMM to send also imm_data
	...
	PSIB_DATA((((psib_msg_t *)con_info->remote_ptr) + con_info->send_pos), len);

    sr_desc.r_key = con_info->remote_rkey;

//    printf("Send to 0x%lx 0x%x magic %08x\n", sr_desc.remote_addr, sr_desc.r_key,
//	   msg->tail.magic);
...
VAPI_ret_t psib_check_cq(hca_info_t *hca_info)
{
	...
	if (comp_desc.opcode == VAPI_CQE_SQ_RDMA_WRITE) {
		...
		if (psib_debug > 0) {
		    printf("Failed RDMA write request (status %d). Connection broken!\n",
			   comp_desc.status);
		}
		con->con_broken = 1;
	    }
...
static
void run_pp_server(hca_info_t *hca, psib_con_info_t *con)
{
    void *rbuf;
    int ret;
    int len;
...
static
int run_pp(hca_info_t *hca, psib_con_info_t *con, int loops, int msize)
{
    int cnt;
...
do_pp(void)
{
	...
	while (1) {
	    ...
	    fflush(stdout);
	    if (scanf("%x %x %lx %lx", &rlid, &rqpn, &rkey, &ptr) == 4) break;
	    printf("wrong format...\n");
	}
	remote_lid = rlid;
	remote_qpn = rqpn;
    ...
    if (psib_connect_con(&con, remote_lid, remote_qpn, remote_ptr, remote_rkey)) goto err_connect_con;
