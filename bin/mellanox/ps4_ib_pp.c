...
static void psib_cleanup_con(hca_info_t *hca_info, con_info_t *con_info)
{
                         ...
                         con_info_t *con_info)
{
    ...
    if (psib_vapi_alloc(hca_info, IB_MTU * SIZE_SR_QUEUE, VAPI_EN_LOCAL_WRITE,
                        &con_info->recv_bufs)) {
        goto err_alloc;
    }
    con_info->recv_pos = 0;
...
static int psib_connect_con(con_info_t *con_info,
...
static int psib_send(con_info_t *con_info, void *buf, int size)
{
    int len;
    psib_msg_t *msg;
...
static int psib_recvdone(con_info_t *con_info)
{
    ...
    rr_desc.id     = (VAPI_wr_id_t)(unsigned long)con_info; /* User defined work
                                                               request ID */
...
static int psib_recvlook(con_info_t *con_info, void **buf)
{
    while (con_info->n_recv_toks > 0) {
        ...
        *buf = &msg->data[0];
        if (msg->header.payload) { return msg->header.payload; }
...
static void run_pp_server(hca_info_t *hca, con_info_t *con)
{
    void *rbuf;
    int ret;
    int len;
...
static int run_pp(hca_info_t *hca, con_info_t *con, int loops, int msize)
{
    int cnt;
...
void do_pp(void)
{
    ...
    if (psib_connect_con(&con, remote_lid, remote_qpn)) {
