...
void msg_hello_bcast(pscom_connection_t *con, void *data, size_t data_len)
{
    ...
    for (c = pscom_get_next_connection(pscom_socket, NULL); c;
         c = pscom_get_next_connection(pscom_socket, c)) {
        if (c != con) {
            pscom_send(c, &(xhead_t){.msg_type = HELLO}, sizeof(xhead_t), data,
                       data_len);
        }
    }
    ...
    for (c = pscom_get_next_connection(pscom_socket, NULL); c;
        ...
        if (c != con) {
            pscom_send(con, &(xhead_t){.msg_type = CLIENT_INFO},
                       sizeof(xhead_t), (void *)c_info, strlen(c_info) + 1);
        }
    }
}
...
void dispatch(void)
{
    ...
    while (1) {
        ...
        case HELLO: msg_hello(req->connection, req->data, req->data_len); break;
        case HELLO_RESPONSE:
...
int main(int argc, char **argv)
{
    ...
    if (!arg_serveraddr) {
        ...
        pscom_send(pscom_con, &(xhead_t){.msg_type = HELLO}, sizeof(xhead_t),
                   "Hello Server", 13);
        pscom_send(pscom_con, &(xhead_t){.msg_type = HELLO_BCAST},
                   sizeof(xhead_t), "Hello World", 13);
    }
    dispatch();
