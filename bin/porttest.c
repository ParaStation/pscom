...
void parse_opt(int argc, char **argv)
{
    ...
    struct poptOption optionsTable[] = {
        {"recvport", 'r', POPT_ARG_INT, &arg_recvport, 0, "port to listen on",
         "port"},
        {"sendport", 's', POPT_ARG_INT, &arg_sendport, 0, "port to send to",
         "port"},
...
static char *inetstr(int addr)
{
    static char ret[16];
    sprintf(ret, "%u.%u.%u.%u", (addr >> 24) & 0xff, (addr >> 16) & 0xff,
...
#define RECVBUFLEN 0x3000000
    ...
    while (1) {
        rh = PSP_IReceive(ph, recvbuf, RECVBUFLEN, &head.phead,
                          sizeof(head.xhead), PSP_RecvAny, 0);
...
#define SENDBUFLEN 0
    ...
    if (arg_sendhost == INADDR_LOOPBACK) { arg_sendhost = PSP_GetNodeID(); }
    ...
    for (i = 0; i < 3; i++) {
        rh = PSP_ISend(ph, sendbuf, SENDBUFLEN,
                       //		    strlen( sendbuf ),
                       &head.phead, sizeof(head.xhead), dest, 0);
        PSP_Wait(ph, rh);
        printf("Send %d done\n", i);
        // usleep(5000000);
    }
