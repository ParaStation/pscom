...
static void parse_opt(int argc, char **argv)
{
    ...
    struct poptOption optionsTable[] = {
        ...
        {"roff", 'r', POPT_ARGFLAG_SHOW_DEFAULT | POPT_ARG_INT, &arg_recvoffset,
         0, "receive buffer offset", "bytes"},
        ...
        {"pscom", 's', POPT_ARGFLAG_OR | POPT_ARG_VAL, &arg_pscom, 1,
         "poll via pscom's psoib_sendv()", NULL},

        {"read", 0, POPT_ARGFLAG_OR | POPT_ARG_VAL, &arg_read, 1,
         "One direction RDMA read (no pp!)", NULL},
...
static void psoib_rc_check(const char *msg, int rc)
{
    if (rc) {
        fprintf(stderr, "%s : %s : %s\n", msg,
                psoib_err_str ? psoib_err_str : "???", strerror(rc));
        exit(1);
    }
}


static inline void x_recv(void)
{
    if (arg_poll_char) {
        ...
        *mark = 0;
        ...
        volatile uint32_t *mark = (uint32_t *)((char *)mcon->recv.bufs.ptr +
                                               arg_recvoffset);
...
static inline void x_send_read(unsigned msgsize)
{
    int rc;
    mem_info_t *send_bufs   = &mcon->send.bufs;
    volatile uint32_t *mark = send_bufs->ptr;
    *mark                   = 0x42424242;
    ...
    struct ibv_send_wr wr = {
        ...
        // (
        //	(ENABLE_SEND_NOTIFICATION ? IBV_SEND_SIGNALED : 0) | /* no cq
        // entry, if unsignaled */
        //	((list.length <= IB_MAX_INLINE) ? IBV_SEND_INLINE : 0)),
        .imm_data   = 42117,
...
static void x_send_pscom(unsigned msglen)
{
    int len;
    struct iovec amsg = {
        .iov_base = abuffer,
    ...
    // Flush cq:
    while (mcon->outstanding_cq_entries) { psoib_progress(); }
}
...
static void info_write(FILE *peer, psoib_info_msg_t *msg)
{
    ...
    fprintf(peer,
            VERSION " lid:%u qp:%u ptr:%p rkey:%u roff:%d pollchar:%d "
                    "pscom:%d\n",
            msg->lid, msg->qp_num, msg->remote_ptr, msg->remote_rkey,
...
static void info_read(FILE *peer, psoib_info_msg_t *msg)
{
    ...
    rc = fscanf(peer,
                VERSION " lid:%u qp:%u ptr:%p rkey:%u roff:%d pollchar:%d "
                        "pscom:%d",
                &a1, &a2, &msg->remote_ptr, &a4, &arg_recvoffset,
                &arg_poll_char, &arg_pscom);
...
static void pscom_openib_init(FILE *peer)
{
    ...
    if (arg_read) {
        // RDMA Read require different MR permissions. Hack: Overwrite buffer
        // allocations:
        ...
        psoib_rc_check("psoib_vapi_alloc(IBV_ACCESS_LOCAL_WRITE, "
                       "&mcon->send.bufs)",
                       rc);

        rc = psoib_vapi_alloc(&psoib_default_hca, IB_MTU * psoib_recvq_size,
                              IBV_ACCESS_REMOTE_READ, &mcon->recv.bufs);
        psoib_rc_check("psoib_vapi_alloc(IBV_ACCESS_REMOTE_READ, "
                       "&mcon->recv.bufs)",
                       rc);
    }
