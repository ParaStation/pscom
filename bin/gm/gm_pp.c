...
void parse_opt(int argc, char **argv)
{
	...
	{ "client" , 'c', POPT_ARGFLAG_OR, &arg_client, 0,
	  "run as client", "" },
	{ "gmport"  , 'p', POPT_ARG_INT, &arg_gmport , 0,
	  "gm port (default 4)", "port" },
...
#else
	do {
	    /* This polling (against gm_blocking_receive()) saves about 0.25 us!*/
...
//    length=gm_max_length_for_size(max_size); /* whats this? */
	...
	printf("Destination is local_id %u, global_id %d\n", target_node, arg_target_node);

	while (our_length > 0) {
	    t1 = getusec();
	    for (sends = 0;sends < arg_loops;sends++){
		gm_send_with_callback(port, s_buffer[s_idx], max_size, our_length,
				      GM_LOW_PRIORITY,
...
//	    printf("Send %d %d \n", max_size, our_length);
	...
	while(1){
	    ...
	    gm_send_with_callback(port, s_buffer[s_idx], max_size,
				  ...
				  send_cb, 0);
//	    printf("Send %d %d \n", max_size, gm_ntohl(event->recv.length));
	    s_idx = (s_idx + 1) % numstokens;
