...
#define nrbuf 8
#define nsbuf 28
    void *rbuffer[nrbuf] = {0,};
    void *sbuffer[nsbuf] = {0,};
    gm_recv_event_t* event;
    int sends=0;
    ...
    if ((status = gm_open(&port, 0, *argv[1]-'0', "bla", GM_API_VERSION_1_1))!= GM_SUCCESS){
...
//		usleep(10000);
	    ...
	    gm_send_with_callback(port, sbuffer[sendn], max_size, our_length,
				  GM_LOW_PRIORITY,
				  arg_target_node,arg_target_port,
				  send_callb,0);
...
//	    fprintf(stderr,"in_tran %d\n",send_in_transit);
	...
	printf("Elapsed usec/send: %f  through %f\n",
	       ((uint32)(t2-t1)) / (1.0*max_num_sends) , (1.0*our_length*max_num_sends) /((uint32)(t2-t1)));
    } else {
	int rcnt =0;
	/* receiver */
	while(1){
