...
enum msg_type {
	MSG_TYPE_REQUEST_TEST = 1,
	MSG_TYPE_MSG_ORDERING = 2
};
...
void parse_opt(int argc, char **argv)
{
	...
	struct poptOption optionsTable[] = {
		{ "server"	, 's', POPT_ARGFLAG_OR | POPT_ARG_VAL,
		  &arg_type	, ARG_SERVER, "run as server", NULL },
	...
	while ((c = poptGetNextOpt(optCon)) >= 0) {
		...
		case 'l': arg_type |= ARG_SERVER; break;
...
int recv_accept_request_test(pscom_request_t *req, pscom_connection_t *con, pscom_header_net_t *header)
{
...
void io_done_msg_ordering(pscom_request_t *req)
{
	printf("### Request %p state %s io_done_msg_ordering (hlen:%3lu, dlen:%4lu) recv_tag %d expect_tag %d idx %d\n",
	       ...
	       req->user->req.msg_ordering.tag,
	       req->user->req.msg_ordering.index);
...
int recv_accept_msg_ordering(pscom_request_t *req, pscom_connection_t *con, pscom_header_net_t *header)
{
	return header->xheader->user.tag == req->user->req.msg_ordering.tag;
}

static
void server_msg_ordering(pscom_connection_t *con)
{
	struct {
	...
	for (i = 0; n[i].tag; i++) {
		pscom_request_t *req = pscom_request_create(
			sizeof(req->xheader), sizeof(req->user->req.msg_ordering) + n[i].data_len);
...
static
void client_msg_ordering(pscom_connection_t *con)
{
	struct {
		int tag;
		int data_len;
