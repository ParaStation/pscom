...
void time_handler_old(int signal)
{
    ...
    for (j = 0; j < ranks_n; j++) { fprintf(stdout, "%1d%s", j % 10, space); }
    fprintf(stdout, "\n");
        ...
        for (k = 0; k < ranks_n; k++) {
            if (!arg_type) {
                ...
                if (ctype == PSCOM_CON_TYPE_NONE) {
                    ...
                    if (ctype < max_used_con_types) {
                        used_con_types[ctype] = 1;
                    }
                }
            }
        }
...
static void set_rlimit_nofile(int rlimit_nofile)
{
    struct rlimit rl;
    int rc;
...
void print_list_sort(int *ilist, int size)
{
    ...
    for (i = 1; i < size; i++) {
        if (silist[sisize - 1] != ilist[i]) { silist[sisize++] = ilist[i]; }
    }
...
int answer_equal(int i, int j)
{
    ...
    for (k = 1; k < arg_np; k++) {
        ...
        if (((conrecv[i][k] > 0) ^ (conrecv[j][k] > 0))) { return 0; }
    }
    return 1;
}
...
static int rank_of_connection(pscom_connection_t *con)
{
    int rank;
    for (rank = 0; rank < maxnp; rank++) {
        if (map_conn[rank] == con) { return rank; }
    }
    return -1;
}
...
static void con_error_handler(pscom_connection_t *connection,
                              pscom_op_t operation, pscom_err_t error)
{
    if (error == PSCOM_ERR_EOF) {
        return; // do not report EOF
    }
...
static void set_myrank(int rank)
{
    myrank = rank;
    char name[10];
...
void send_msg(pscom_connection_t *connection, void *xheader,
              unsigned int xheader_len, void *data, unsigned int data_len)
{
    if (!connection) { return; }

    pscom_send(connection, xheader, xheader_len, data, data_len);
}
...
static void recv_info_from_master()
{
    if (myrank == -1) {
        pscom_err_t err;
        int rank;

        err = pscom_recv_from(map_conn[0], NULL, 0, &rank, sizeof(myrank));
...
static void spawn_manual_master(int argc, char **argv, int np)
{
...
static void spawn_manual(int argc, char **argv, int np)
{
    if (i_am_server) {
        set_myrank(0);
        master_port = pscom_get_portno(pscom_socket);
        printf("Call:\n%s %s\n", argv[0],
               pscom_socket_str(master_node, master_port));
    }
}


static void spawn_master(int argc, char **argv, int np)
{
...
static void spawn_pse(int argc, char **argv, int np)
{
#ifdef PSMGMT_ENABLED
    if (myrank == 0) {
        /* Master node: Set parameter from rank 0 */
        PSE_spawnTasks(np - 1, pscom_get_nodeid(),
                       pscom_get_portno(pscom_socket), argc, argv);
    }
...
static void spawn(int argc, char **argv, int np)
{
    if (arg_manual) {
...
void run(int argc, char **argv, int np)
{
    ...
    if (pscom_listen(pscom_socket, arg_port)) {
        if ((errno == EADDRINUSE) && i_am_server && (arg_port == DEFAULT_PORT)) {
            // server already started? Try again as a client with rank > 0 with
            // any port.
            i_am_server = 0;
...
int main(int argc, char *argv[])
{
    ...
    struct poptOption optionsTable[] = {
        {"np", '\0', POPT_ARG_INT | POPT_ARGFLAG_ONEDASH, &arg_np, 0,
         "number of processes to start", "num"},
        {"cnt", '\0', POPT_ARG_INT | POPT_ARGFLAG_ONEDASH, &arg_cnt, 0,
         "number of test messages to send", "num"},
        {"map", '\0', POPT_ARG_NONE | POPT_ARGFLAG_ONEDASH, &arg_map, 0,
         "print map", NULL},
        {"type", 't', POPT_ARG_NONE | POPT_ARGFLAG_ONEDASH, &arg_type, 0,
         "print connection type", NULL},
        {"manual", 'm', POPT_ARG_NONE | POPT_ARGFLAG_ONEDASH, &arg_manual, 0,
         "manual processes start", NULL},
        {"listen", 'l',
         POPT_ARGFLAG_SHOW_DEFAULT | POPT_ARGFLAG_ONEDASH | POPT_ARG_INT,
