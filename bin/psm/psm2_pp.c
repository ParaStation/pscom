...
static void parse_opt(int argc, char **argv)
{
    ...
    struct poptOption optionsTable[] = {
        ...
        {"uuid", 0, POPT_ARGFLAG_SHOW_DEFAULT | POPT_ARG_INT, &arg_uuid, 0,
         "uuid seed (one byte only)", "uint8"},
...
static void pspsm_ret_check(psm2_error_t ret, char *msg)
{
    if (ret == PSM2_OK) { return; }

    fprintf(stderr, "%s : %s\n", msg, psm2_error_get_string(ret));
    exit(1);
}
...
static char *pspsm_uuid_str(psm2_uuid_t uuid)
{
...
static void pspsm_connect(void)
{
    ...
    ret = psm2_ep_connect(my_ep, 1, &remote_epid, NULL, errors, &remote_epaddr,
                          0);
    pspsm_ret_check(ret, "psm2_ep_connect()");
}


static void pspsm_disconnect(void)
{
#ifdef PSM2_EP_DISCONNECT_FORCE
    psm2_error_t ret;
    psm2_error_t err = PSM2_OK;

    ret = psm2_ep_disconnect2(my_ep, 1, &remote_epaddr, NULL, &err,
...
static inline void pspsm_send(size_t len)
{
    ...
    ret = psm2_mq_send(my_mq, remote_epaddr, MQ_FLAGS_NONE, /* no flags, not a
                                                               sync send */
...
static inline void pspsm_send_eof(void)
{
    psm2_error_t ret;
    size_t slen;
...
static inline unsigned pspsm_recv(void)
{
    ...
    ret = psm2_mq_irecv(my_mq, MQ_TAG, /* don't care tag */
