...
psm_epid_t remote_epid;     // from the info message
psm_epaddr_t remote_epaddr; // set by psm_ep_connect(.. remote_epid ...)
psm_uuid_t remote_uuid;     // from the info message
...
static void pspsm_ret_check(psm_error_t ret, char *msg)
{
    if (ret == PSM_OK) { return; }

    fprintf(stderr, "%s : %s\n", msg, psm_error_get_string(ret));
    exit(1);
}
...
static char *pspsm_uuid_str(psm_uuid_t uuid)
{
    static char uuid_str[16 * 3 + 4];
    snprintf(uuid_str, sizeof(uuid_str), PSPSM_UUID_FMT, uuid[0], uuid[1],
             uuid[2], uuid[3], uuid[4], uuid[5], uuid[6], uuid[7], uuid[8],
             uuid[9], uuid[10], uuid[11], uuid[12], uuid[13], uuid[14],
             uuid[15]);
    return uuid_str;
}
...
static void pspsm_init_con(void)
{
    psm_error_t ret;
    pspsm_init_bufs();

    ret = psm_ep_open(my_uuid, NULL, &my_ep, &my_epid);
    pspsm_ret_check(ret, "psm_ep_open()");

    ret = psm_mq_init(my_ep, PSM_MQ_ORDERMASK_ALL, NULL, 0, &my_mq);
    pspsm_ret_check(ret, "psm_mq_init()");
}

#define SEC_IN_NS 1000000000ULL

static void pspsm_connect(void)
{
    psm_error_t ret;
    psm_error_t errors[1];

    ret = psm_ep_connect(my_ep, 1, &remote_epid, NULL, errors, &remote_epaddr,
                         0);
    pspsm_ret_check(ret, "psm_ep_connect()");
}


static void pspsm_init_uuid(void)
{
...
static void pp_info_read(FILE *peer, pp_info_msg_t *msg)
{
    ...
    rc = fscanf(peer,
                VERSION " len:%d uuid:" PSPSM_UUID_FMT " epid:" PSPSM_EPID_FMT,
                &msg->len, &msg->uuid[0], &msg->uuid[1], &msg->uuid[2],
                &msg->uuid[3], &msg->uuid[4], &msg->uuid[5], &msg->uuid[6],
                &msg->uuid[7], &msg->uuid[8], &msg->uuid[9], &msg->uuid[10],
                &msg->uuid[11], &msg->uuid[12], &msg->uuid[13], &msg->uuid[14],
                &msg->uuid[15], &msg->epid);
    if (rc != 18) {
        error(1, 0,
              "Parsing error! Only %d from 18 fields. Version mismatch?\n", rc);
    }
...
static inline void pspsm_send(unsigned len)
{
    psm_error_t ret;
    unsigned slen;
    ...
    ret = psm_mq_send(my_mq, remote_epaddr, MQ_FLAGS_NONE, /* no flags, not a
                                                              sync send */
...
static inline unsigned pspsm_recv(void)
{
    ...
    ret = psm_mq_irecv(my_mq, MQ_TAG, /* don't care tag */
                       MQ_TAGSEL_ALL, /* always successfully tag match */
                       MQ_FLAGS_NONE, /* no flags */
                       r_buf, sizeof(*r_buf), MQ_NO_CONTEXT_PTR, &req);
    pspsm_ret_check(ret, "psm_mq_irecv");

    ret = psm_mq_wait(&req, &status);
    pspsm_ret_check(ret, "psm_mq_wait");
