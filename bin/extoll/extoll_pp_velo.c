...
static void extoll_ret_check(enum velo2_ret ret, char *msg)
{
    ...
    case VELO2_RET_INVALID_NODE:
        ...
        break;
    default: fprintf(stderr, "%s : Unknown velo2 error %d\n", msg, ret);
    }
    exit(1);
}
...
static void init(FILE *peer)
{
    ...
    ret = velo2_connect(&velo2_port, &remote_connection, remote_nodeid,
                        remote_vpid);
...
static inline void extoll_send(unsigned len)
{
    ...
    while (slen) {
        ...
        ret = velo2_send(&remote_connection, s, next_len, 0x00, 0);
...
static inline unsigned extoll_recv(void)
{
    enum velo2_ret ret;
    ...
    ret = velo2_recv(&velo2_port, r, 64, &mlen, &sourceid, &tag, 0);
