...
static void parse_opt(int argc, char **argv)
{
    ...
    struct poptOption optionsTable[] = {
        {"soffset", 's', POPT_ARGFLAG_SHOW_DEFAULT | POPT_ARG_INT, &arg_soffset,
         0, "s_buf offset", "offset"},
        {"roffset", 'r', POPT_ARGFLAG_SHOW_DEFAULT | POPT_ARG_INT, &arg_roffset,
         0, "r_buf offset", "offset"},
        {"bytes", 'b', POPT_ARGFLAG_SHOW_DEFAULT | POPT_ARG_INT, &arg_bytes, 0,
         "bytes to send", "count"},
...
static void dump_msg(void *buf, int offset, int size)
{
    while (offset < size) {
...
static void run_server(void)
{
    ...
    rc = rma2_post_put_bt(extoll_port, remote_handle, extoll_s_region,
                          arg_soffset, arg_bytes, remote_rbuf + arg_roffset,
...
int main(int argc, char **argv)
{
    ...
    if (arg_ssh_clienthost) {
        ...
        snprintf(cmd, sizeof(cmd),
                 "bash -x -c \"ssh %s 'cd %s && %s --bytes=%u $(hostname "
                 "-s)'\" &",
                 arg_ssh_clienthost, getcwd(wd, sizeof(wd)), argv[0],
                 arg_bytes);
