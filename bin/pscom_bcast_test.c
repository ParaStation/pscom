...
static void parse_opt(int argc, char **argv)
{
    ...
    struct poptOption optionsTable[] = {
        {"np", 'n', POPT_ARGFLAG_SHOW_DEFAULT | POPT_ARG_INT, &arg_np, 'n',
         "number of procs to wait for", "np"},
...
static void hello_received(pscom_request_t *req)
{
    unsigned con_idx;
    ...
    if (arg_verbose) {
        printf("server receive: %s : %s : np = %d\n", Con->hello.vc,
               Con->hello.buf, Con->hello.np);
    }

    if (Con->hello.np != arg_np) {
        printf("Reject connection from %s : wrong np (%d) should be (%d)\n",
               pscom_con_info_str(&con->remote_con_info), Con->hello.np,
               arg_np);
...
static void do_accept(pscom_connection_t *con)
{
    ...
    if (!connections) { connections = malloc(sizeof(*connections) * arg_np); }
...
static void do_accept_client(pscom_connection_t *con)
{
    unsigned rank;
    pscom_err_t rc;
...
static void bcast_test(unsigned buflen, unsigned groot)
{
...
static void bcast_test2(unsigned xlen, unsigned buflen, unsigned groot)
{
    ...
    pscom_bcast(group, groot, xbuf, xlen, buf, buflen);


    printf("%5d : rank %2d:received:(%s)%s\n", getpid(), my_rank, xbuf, buf);
    fflush(stdout);
...
int main(int argc, char **argv)
{
    ...
    while (connections_client_count != arg_np) { pscom_wait_any(); }
    ...
    */
    ...
    group = pscom_group_open(sock_client, 42, my_rank, arg_np,
                             connections_client);
    ...
    for (i = 0; i < 5; i++) { bcast_test(100000, pscom_min(arg_np - 1, 6)); }

    bcast_test2(40, 400, pscom_min(arg_np - 1, 3));
    bcast_test2(40, 400, pscom_min(arg_np - 1, 1));

    while (0) { pscom_wait_any(); }

    if (arg_verbose) { printf("waiting on barrier (rank %d)\n", my_rank); }

    //	if (my_rank == 1) sleep(4);

    pscom_barrier(group);

    if (arg_verbose) { printf("barrier done       (rank %d)\n", my_rank); }
