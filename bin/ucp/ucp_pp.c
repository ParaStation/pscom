 ...
 * Copyright (C) 2005-2010,2016 ParTec Cluster Competence Center GmbH, Munich
 *
...
void psucp_init(void) {
	...
	status = ucp_worker_get_address(ucp_worker, &local_addr.ucp_address, &local_addr.size);
	assert(status == UCS_OK);
}
...
static
void send_done(void *request, ucs_status_t status) {
	TRACE(printf("%s:%u:%s\n", __FILE__, __LINE__, __func__));
}


static
void recv_done(void *request, ucs_status_t status, ucp_tag_recv_info_t *info) {
	TRACE(printf("%s:%u:%s\n", __FILE__, __LINE__, __func__));
}
...
unsigned myucp_recv(void)
{
	...
	while (1) {
		msg_tag = ucp_tag_probe_nb(ucp_worker, UCP_TAG,
					   (ucp_tag_t)~0 /* tag bit mask */, 1 /* remove */,
					   &info_tag); /* What an ugly API :-( */
		if (msg_tag != NULL) {
			break;
		}
		ucp_worker_progress(ucp_worker);
	...
	TRACE(printf("%s:%u:%s info_tag.length %zu\n", __FILE__, __LINE__, __func__, info_tag.length));
