...
| `PSP_PRECON_TYPE`
| Choose TCP/IP or RRComm as precon protocol. The default is TCP/IP (`PSP_PRECON_TYPE=0`). When RRComm is enabled (`PSP_PRECON_TYPE=1`), the pscom4tcp plugin is automatically disabled (`PSP_TCP=0`) and `PSP_GUARD` has to be set to 0.
