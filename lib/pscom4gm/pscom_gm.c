...
static int _pscom_gm_do_read(psgm_sock_t *sock)
{
    void *con_id;
    void *buf;
    unsigned int size;
...
static int pscom_gm_do_read(pscom_poll_t *poll)
{
...
static void pscom_gm_do_write(pscom_con_t *con)
{
    unsigned int len;
    struct iovec iov[2];
    pscom_req_t *req;
...
static void pscom_gm_read_start(pscom_con_t *con)
{
...
static void pscom_gm_read_stop(pscom_con_t *con)
{
    if (con->arch.gm.reading) {
        psgm_sock_t *sock = &get_sock(con->pub.socket)->gm;
...
static void pscom_gm_close(pscom_con_t *con)
{
    psgm_con_info_t *gmcon = con->arch.gm.gmcon;
...
static void pscom_gm_con_init(pscom_con_t *con, int con_fd,
...
/*********************************************************************/
static void pscom_gm_init(void)
{
    psgm_debug = pscom.env.debug;
}


static void pscom_gm_sock_init(pscom_sock_t *sock)
{
    sock->gm.readers   = 0;
...
static int pscom_gm_connect(pscom_con_t *con, int con_fd)
{
    ...
    if (psgm_init() || !gmcon) { goto dont_use; /* Dont use gm */ }
...
static int pscom_gm_accept(pscom_con_t *con, int con_fd)
{
    int arch               = PSCOM_ARCH_GM;
    psgm_con_info_t *gmcon = NULL;
    psgm_info_msg_t msg;

    if (psgm_init()) { goto out_nogm; }

    gmcon = psgm_con_create();
    if (!gmcon) { goto out_nogm; }

    if (psgm_con_init(gmcon, NULL)) { goto err_con_init; }
    ...
    if (psgm_con_connect(gmcon, NULL, &msg)) { goto err_connect_con; }
