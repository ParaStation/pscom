...
pscom_ofed_env_parser_set_pending_tokens(void *buf, const char *config_val)
{
    const char *set_val = config_val ? config_val
                                     : psofed_pending_tokens_suggestion_str();
...
static pscom_env_table_entry_t pscom_env_table_ofed[] = {
    {"HCA", NULL,
     ...
     "hca).",
     &psofed_hca, PSCOM_ENV_ENTRY_FLAGS_EMPTY, PSCOM_ENV_PARSER_STR},

    {"PORT", NULL, "Port to use (default is first active port).", &psofed_port,
     PSCOM_ENV_ENTRY_FLAGS_EMPTY, PSCOM_ENV_PARSER_UINT},

    {"PATH_MTU", "3", "MTU of the IB packets. (1:256, 2:512, 3:1024.",
     &psofed_path_mtu, PSCOM_ENV_ENTRY_FLAGS_EMPTY, PSCOM_ENV_PARSER_UINT},

    {"COMPQ_SIZE", "2048", "Size of the completion queue queue.",
     &psofed_compq_size, PSCOM_ENV_ENTRY_FLAGS_EMPTY, PSCOM_ENV_PARSER_UINT},

    {"RECVQ_SIZE", "1024", "Number of receive buffers per connection.",
     &psofed_recvq_size, PSCOM_ENV_ENTRY_FLAGS_EMPTY, PSCOM_ENV_PARSER_UINT},

    {"SENDQ_SIZE", "1024", "Number of send buffers per connection.",
     &psofed_sendq_size, PSCOM_ENV_ENTRY_FLAGS_EMPTY, PSCOM_ENV_PARSER_UINT},
    ...
    {"WINSIZE", "512", "Maximum number of unacked packets.", &psofed_winsize,
     PSCOM_ENV_ENTRY_FLAGS_EMPTY, PSCOM_ENV_PARSER_UINT},
    ...
    {"RESEND_TIMEOUT", "10000",
     ...
     &psofed_resend_timeout, PSCOM_ENV_ENTRY_FLAGS_EMPTY, PSCOM_ENV_PARSER_UINT},
    ...
    {"EVENT_CNT", "1",
     ...
     &psofed_event_count, PSCOM_ENV_ENTRY_FLAGS_EMPTY, PSCOM_ENV_PARSER_INT},

    {"LID_OFFSET", "0", "Offset to base LID (adaptive routing).",
     &psofed_lid_offset, PSCOM_ENV_ENTRY_FLAGS_EMPTY, PSCOM_ENV_PARSER_INT},
...
static int pscom_ofed_do_read(pscom_poll_t *poll);
...
static void reader_inc(void)
{
    if (!pscom_ofed.reader_user) {
        ...
        pscom_poll_start(&pscom_ofed.poll_read, pscom_ofed_do_read,
                         &pscom.poll_read);
    }
...
static void pscom_ofed_read_start(pscom_con_t *con)
{
...
static void pscom_ofed_read_stop(pscom_con_t *con)
{
    if (con->arch.ofed.reading) {
...
static int pscom_ofed_do_write(pscom_poll_t *poll)
{
    struct iovec iov[2];
    pscom_req_t *req;
...
static void pscom_ofed_con_cleanup(pscom_con_t *con)
{
    psofed_con_info_t *mcon = con->arch.ofed.mcon;
    if (!mcon) { return; }
...
static void pscom_ofed_con_close(pscom_con_t *con)
{
    psofed_con_info_t *mcon = con->arch.ofed.mcon;
    if (!mcon) { return; }
...
static void pscom_poll_write_start_ofed(pscom_con_t *con)
{
    pscom_poll_write_start(con, pscom_ofed_do_write);
}


static void pscom_ofed_init_con(pscom_con_t *con)
{
...
/*********************************************************************/
static void pscom_ofed_init(void)
{
...
static int pscom_ofed_con_init(pscom_con_t *con)
{
    return psofed_init();
}
                                 ...
                                 unsigned size)
{
    ...
    case PSCOM_INFO_OFED_ID: {
        ...
        if (psofed_con_connect(con->arch.ofed.mcon, msg)) {
    ...
    case PSCOM_INFO_EOF: pscom_ofed_init_con(con); break; /* Done. Use Ofed */
    }
    return;
    /* --- */
error_con_connect:
