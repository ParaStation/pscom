...
#define IB_MTU	(2*1024) /* must be <= 2048 (IB UD MTU limit), should be a power of 2 */
...
struct context_info {
	...
	unsigned int		recv_done;		// count receives which are waiting for further processing.

	/* connections */
	psofed_con_info_t	**connections;		// list of all connections (indexed by "src")
...
unsigned int psofed_pending_tokens = _SIZE_RECV_QUEUE / 3; // Send ACK if pending_tokens are outstanding
unsigned int psofed_winsize = _SIZE_RECV_QUEUE / 2; // Do not send more then winsize unacked messsages

unsigned long psofed_resend_timeout = 10000; // resend in usec. 4 times the timeout on each resend starting with psofed_resend_timeout
...
int timestamp_expired(unsigned long timestamp, unsigned long now, unsigned resendcnt)
{
	...
	unsigned long expire_delta = psofed_resend_timeout <<
		((shift < psofed_resend_timeout_shift) ?
		 shift : psofed_resend_timeout_shift); // doubling timeout each resend.
...
static
struct ibv_context *psofed_open_hca(char *hca_name)
{
...
static
int psofed_start_qp(struct ibv_qp *qp)
{
	int rc;
	/* init -> rtr */
	{
...
static
void psofed_fix_compq_size(struct ibv_context *ctx)
{
	struct ibv_device_attr device_attr;
...
static
void psofed_vapi_free(context_info_t *context, mem_info_t *mem_info)
{
...
void print_mlock_help(unsigned size)
{
	...
	psofed_dprint(0, "(Check memlock limit in /etc/security/limits.conf or try 'ulimit -l')");

	if (!getrlimit(RLIMIT_MEMLOCK, &rlim)) {
		psofed_dprint(0, "Current RLIMIT_MEMLOCK: soft=%lu byte, hard=%lu byte", rlim.rlim_cur, rlim.rlim_max);
	}
}

static
int psofed_vapi_alloc(context_info_t *context, int size, enum ibv_access_flags access_perm, mem_info_t *mem_info)
{
	mem_info->mr = NULL;
...
static
void _psofed_post_recv(context_info_t *context)
{
	psofed_msg_t *msg = ((psofed_msg_t*)context->recvq.bufs.ptr) +
		(context->recvq.pos +
		 context->recv_posted) % psofed_recvq_size;
...
//	       context->recv_posted);
	...
	if (rc) {
		psofed_dprint(0, "ibv_post_recv() failed!");
	}
...
static
void psofed_post_recvs(context_info_t *context)
{
	unsigned i;
...
static
int psofed_con_assign_src(psofed_con_info_t *con_info)
{
	unsigned idx;
	...
	nconnections = realloc(context->connections,
			       context->connections_count * sizeof(context->connections[0]));
 ...
 * return -1 on error */
static
int psofed_con2src(psofed_con_info_t *con_info)
{
	unsigned idx;
	context_info_t *context = con_info->context;
	...
	for (idx = 0;
	     idx < context->connections_count; idx++) {
		if (context->connections[idx] == con_info)
			return idx;
	}
	return -1;
}
...
static
void psofed_con_unassign_src(psofed_con_info_t *con_info)
{
	int idx;
	context_info_t *context = con_info->context;
...
static
psofed_send_buffer_t *get_send_buffer(context_info_t *context)
{
	psofed_send_buffer_t *sbuf;

	if (list_empty(&context->sbuf_pool)) {
		if (!psofed_progress(context) ||
		    list_empty(&context->sbuf_pool)) {
			return NULL;
		}
	}
...
static
void put_send_buffer(context_info_t *context, psofed_send_buffer_t *sbuf)
{
...
err_double_free:
	psofed_dprint(0, "OFED stack bug: ibv_poll_cq(cq, 1, &wc)"
		      " return opcode IBV_WC_SEND with same work request twice.");
}


static
void psofed_cleanup_context(context_info_t *context)
{
	unsigned i;
...
static
int psofed_init_context(context_info_t *context)
{
	int rc;
	unsigned i;
	...
	rc = psofed_vapi_alloc(context, IB_MTU * psofed_sendq_size,
			       0, &context->send_buffers);
...
static inline
int send_send_buffer(psofed_con_info_t *con_info, psofed_send_buffer_t *sbuf)
{
	int rc;
	context_info_t *context = con_info->context;
	...
	struct ibv_send_wr wr = {
		.wr_id	    = (uint64_t)(unsigned long)sbuf,
		.sg_list    = &list,
		.num_sge    = 1,
		.opcode     = IBV_WR_SEND,
...
static
void sched_resend_send_buffer(psofed_con_info_t *con_info, psofed_send_buffer_t *sbuf)
{
...
static
void resend_con_sendbuffers(psofed_con_info_t *con_info, unsigned long now)
{
	struct list_head *pos;

	if (!timestamp_expired(con_info->last_send, now, con_info->resend_count)) return;
	...
	/* Send all message not in state SBUF_SENDING again. Sending all messages,
	   ...
	   also all following messages need a resend, until we implement an out of order
	   receive queue.

	   Improvement 1: Implement NACKs and send last messsage instead of first. If the
	   receiver detect a lost message, he can request the lost message and all messages
	   afterwards with one NACK.

	   Improvement 2: Additional implement an out ouf order receive queue. The receiver
	   than can request only the lost messages (instead of all messages starting from
	   the lost one) with one NACK.

	 */
	list_for_each(pos, &con_info->resendq) {
		psofed_send_buffer_t *sbuf = list_entry(pos, psofed_send_buffer_t, next);

		if (sbuf->state & SBUF_SENDING) continue; // already sending

		send_send_buffer(con_info, sbuf); /* error ignored. In case of an error,
						     SBUF_SENDING will not be set and send_send()
						     will be called again in the next loop. */
		psofed_stat.send_resends++;
	}

	con_info->last_send = now; // update the timestamp. Even if we didnt send anything.
}


static
void resend(context_info_t *context)
{
	struct list_head *pos;
	unsigned long now;
...
static
void psofed_send_ack(psofed_con_info_t *con_info)
{
	context_info_t *context = con_info->context;
	psofed_send_buffer_t *sbuf;
	psofed_msg_t *msg;
	...
	msg = sbuf->msg;
	msg->header.src	= con_info->src;
	msg->header.seq = con_info->s_acked; // Use an already acked seqno with len==0 as the ACK
	msg->header.len = 0;

	send_send_buffer(con_info, sbuf); /* Ignore errors here. We automatically recover
					     from lost ACKs. */
...
static
void ack_send_buffers(psofed_con_info_t *con_info, psofed_seqno_t ack)
{
	...
	while (!list_empty(&con_info->resendq)) {
		psofed_send_buffer_t *sbuf = list_entry(con_info->resendq.next, psofed_send_buffer_t, next);

		if (psofed_seqcmp(sbuf->msg->header.seq, ack) <= 0) {
			list_del(&sbuf->next);
			sbuf->state |= SBUF_ACKED;
			if (!(sbuf->state & SBUF_SENDING)) put_send_buffer(con_info->context, sbuf);
		} else {
			break;
		}
	}
...
retry:
	...
	psofed_msg_t *msg = ((psofed_msg_t*)((char *)context->recvq.bufs.ptr + IB_UD_OFFSET)) +
		context->recvq.pos;
...
//	       dumpstr(msg->data, MIN(msg->header.len, 16)));
	...
	if (src >= context->connections_count ||
		...
		psofed_dprint(3, "psofed_recv(): drop msg with unkown src = %d (count = %u)",
	...
	TRACE(psofed_dprint(0, "RECV%s Seq:%u Ack:%u len:%u\n",
			    (msg->header.seq == con_info->r_seq) ? "" : (msg->header.len ? "(dup)" : " ACK"),
			    msg->header.seq,
			    msg->header.ack,
			    msg->header.len));
	...
	if (msg->header.seq != con_info->r_seq) {
		...
		if (msg->header.len) {
			if (psofed_seqcmp(msg->header.seq, con_info->r_seq) < 0) {
				...
				// ToDo: Send NACK
				TRACE(psofed_dprint(1, "psofed_recv(): recv out of order msg : seq = %u (expect %u)",
						    (unsigned)msg->header.seq, (unsigned)con_info->r_seq));
			}
		} else {
...
static
void check_resend(psofed_con_info_t *con_info)
{
...
static
int psofed_check_cq(context_info_t *context)
{
	struct ibv_wc wc;
	int rc;
	...
	if (rc == 1) {
		...
		case IBV_WC_SEND:
			if (wc.status != IBV_WC_SUCCESS) {
				psofed_dprint(0, "ibv_poll_cq() : IBV_WC_SEND with status %d",
					      wc.status);
				// ToDo: terminate corresponding connection with error
			}
			psofed_send_buffer_t *sbuf = (psofed_send_buffer_t *)(unsigned long)wc.wr_id;
			...
			if (unlikely(sbuf->state & SBUF_ACKED)) put_send_buffer(context, sbuf);

			check_resend(con_info);
			break;
		default:
			psofed_dprint(0, "ibv_poll_cq() : Unknown opcode: %d", wc.opcode);
		}
...
static
int psofed_poll(context_info_t *context, int blocking)
{
	int rc;
...
void psofed_con_get_info_msg(psofed_con_info_t *con_info /* in */, psofed_info_msg_t *info_msg /* out */)
{
	info_msg->version = PSOFED_INFO_VERSION;
