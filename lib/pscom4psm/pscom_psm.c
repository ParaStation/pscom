...
pscom_env_table_entry_t pscom_env_table_psm[] = {
    ...
    {"PSM_UNIQ_ID", "0",
     ...
     &pscom.env.psm_uniq_id, PSCOM_ENV_ENTRY_FLAGS_EMPTY, PSCOM_ENV_PARSER_UINT},

    {"DEVCHECK", "1",
     "Enable/disable checking for any of the following device files:"
     "/dev/ipath{,0,1},/dev/hfi{1,2}{,_0,_1,_2}",
     &pspsm_devcheck, PSCOM_ENV_ENTRY_FLAGS_EMPTY, PSCOM_ENV_PARSER_UINT},
...
static int pscom_psm_make_progress(pscom_poll_t *poll);
...
static void poll_user_inc(void)
{
    if (!pspsm_poll.poll_user) {
        ...
        pscom_poll_start(&pspsm_poll.poll_read, pscom_psm_make_progress,
                         &pscom.poll_read);
    }
...
static void pscom_psm_read_start(pscom_con_t *con)
{
...
static void pscom_psm_read_stop(pscom_con_t *con)
{
    if (con->arch.psm.reading) {
...
static void pscom_psm_post_recv_check(pscom_con_t *con)
{
    if (con->arch.psm.reading) {
...
static int pscom_psm_do_write(pscom_poll_t *poll)
{
...
static volatile unsigned cleanup_wait_count = 0;

static void pscom_psm_con_cleanup_delayed(void *ci_priv)
{
...
static void pscom_psm_con_cleanup(pscom_con_t *con)
{
    pspsm_con_info_t *ci = con->arch.psm.ci;
    if (!ci) { return; }
    ...
    if (pscom.env.psm_close_delay) {
        pspsm_progress();
        pscom_timer(pscom.env.psm_close_delay, pscom_psm_con_cleanup_delayed,
                    ci);
    } else {
...
static void pscom_psm_con_close(pscom_con_t *con)
{
    pspsm_con_info_t *ci = con->arch.psm.ci;
    if (!ci) { return; }
...
static void pscom_poll_write_start_psm(pscom_con_t *con)
{
    pscom_poll_write_start(con, pscom_psm_do_write);
}


static void pscom_psm_init_con(pscom_con_t *con)
{
...
static void pscom_psm_init(void)
{
    ...
    pscom_env_table_register_and_parse("pscom PSM", "PSM_", pscom_env_table_psm);
    ...
    // Preinitialize pspsm. Ignore errors. pspsm_connect will see the error
    // again.
...
static int pscom_psm_con_init(pscom_con_t *con)
{
    return pspsm_init();
}
                                ...
                                unsigned size)
{
    ...
    case PSCOM_INFO_PSM_ID: {
        ...
        if (pspsm_con_connect(con->arch.psm.ci, msg)) {
    ...
    case PSCOM_INFO_EOF: pscom_psm_init_con(con); break; /* Done. Use Psm */
    }
    return;
    /* --- */
error_con_connect:
...
{
    pspsm_dprint(D_DBG_V, "pspsm_psm_finalize wait for close (%u)",
                 cleanup_wait_count);
