...
#define pspsm_dprint(level, fmt, arg...)                                       \
    do {                                                                       \
        if ((level) <= pspsm_debug) {                                          \
            fprintf(pspsm_debug_stream ? pspsm_debug_stream : stderr,          \
                    "psm:" fmt "\n", ##arg);                                   \
        }                                                                      \
    } while (0)
...
static void pspsm_con_get_info_msg(pspsm_con_info_t *con_info /* in */,
                                   pspsm_info_msg_t *info /* out */);

static int pspsm_recv_start(pspsm_con_info_t *con_info, char *rbuf,
                            size_t rbuflen);
static int pspsm_recv_pending(pspsm_con_info_t *con_info);

/* pspsm_sendv sends an iov. FIXME: returns 0 if the send is complete, -EAGAIN
   if it created one or more requests for it, and -EPIPE in case of an error. */
