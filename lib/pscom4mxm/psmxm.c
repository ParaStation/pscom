...
typedef struct mxm_socket {
    ...
    struct list_head recvq; /* list of posted recv requests :
                               psmxm_recv_req_t.next */
...
static mxm_socket_t mxm_socket;

/*
 * Parameters
 */
struct {
    int flag_no_optimization : 1; // if true: Don't configure MXM for maximal
                                  // performance, be more portable.
} psmxm_params = {
...
static void psmxm_err(const char *str)
{
...
static void psmxm_msg_err(const char *msg, mxm_error_t err)
{
    char buf[400];
...
static int psmxm_check_dev(void)
{
    struct stat s;
    const char **df;
    const char *devfiles[] = {"/sys/class/infiniband/mlx5_0",
                              "/sys/class/infiniband/mlx5_1",
                              "/sys/class/infiniband/mlx5_2", NULL};
...
static void psmxm_init_req_buffer(mxm_req_base_t *req, mxm_socket_t *mxm_socket,
                                  mxm_conn_h mxm_conn, void *data,
                                  unsigned data_len)
{
...
static void psmxm_init_req_iov(mxm_req_base_t *req, mxm_socket_t *mxm_socket,
                               mxm_conn_h mxm_conn, mxm_req_buffer_t *data_iov,
                               unsigned data_iov_count)
{
    /* Initialize request fields */
    req->state        = MXM_REQ_NEW;
    req->mq           = mxm_socket->mxm_mq;
...
static void psmxm_init_mxm_recv_req(mxm_recv_req_t *rreq,
...
static void psmxm_init_mxm_send_req(mxm_send_req_t *sreq,
                                    mxm_socket_t *mxm_socket,
                                    mxm_conn_h mxm_conn,
                                    mxm_req_buffer_t *data_iov,
                                    unsigned data_iov_count)
{
    psmxm_init_req_iov(&sreq->base, mxm_socket, mxm_conn, data_iov,
                       data_iov_count);
    sreq->flags = 0;
...
static void psmxm_init_recv_req(psmxm_recv_req_t *rreq, mxm_socket_t *mxm_socket)
{
    psmxm_init_mxm_recv_req(&rreq->mxm_rreq, mxm_socket, NULL, rreq->data,
                            PSMXM_MTU);
}


static void psmxm_init_send_req(psmxm_send_req_t *sreq,
                                mxm_socket_t *mxm_socket,
                                psmxm_con_info_t *con_info)
{
    psmxm_init_mxm_send_req(&sreq->mxm_sreq, mxm_socket, con_info->mxm_conn,
                            sreq->iov, 2);
}


static void psmxm_recv_req_post(psmxm_recv_req_t *rreq)
{
    mxm_error_t error;
...
static psmxm_recv_req_t *psmxm_recvq_head(void)
{
    return list_entry(mxm_socket.recvq.next, psmxm_recv_req_t, next);
}


static void psmxm_recvq_append(psmxm_recv_req_t *rreq)
{
    list_add_tail(&rreq->next, &mxm_socket.recvq);
}


static void psmxm_init_endpoint(mxm_socket_t *mxm_socket)
{
...
static void psmxm_post_recv_buffers(mxm_socket_t *mxm_socket)
{
    int i;

    if (!list_empty(&mxm_socket->recvq)) {
...
static int psmxm_open_endpoint(mxm_socket_t *mxm_socket)
{
                      ...
                      void *ctx)
{
    ...
    error = mxm_ep_connect(mxm_socket.mxm_ep, info_msg->mxm_ep_addr,
                           &con_info->mxm_conn);
...
err_dev_knem:
    psmxm_dprint(2, "psmxm_init: No \"/dev/knem\" found. Arch mxm is "
                    "disabled.");
    goto err_exit;
err_open:
err_exit:
