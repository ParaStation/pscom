...
static pscom_env_table_entry_t pscom_env_table_mxm[] = {
    {"MXM_DEVCHECK", "1",
     ...
     &psmxm_devcheck, PSCOM_ENV_ENTRY_FLAGS_EMPTY, PSCOM_ENV_PARSER_UINT},
...
static void pscom_mxm_read_start(pscom_con_t *con)
{
...
static void pscom_mxm_read_stop(pscom_con_t *con)
{
    if (con->arch.mxm.reading) {
...
static void pscom_process_rreq(psmxm_recv_req_t *rreq)
{
...
static int pscom_mxm_make_progress(pscom_poll_t *poll)
{
...
static void pscom_mxm_do_write(pscom_con_t *con)
{
    struct iovec iov[2];
...
static void pscom_mxm_con_cleanup(pscom_con_t *con)
{
    psmxm_con_info_t *ci = con->arch.mxm.ci;
    if (!ci) { return; }
...
static void pscom_mxm_con_close(pscom_con_t *con)
{
    psmxm_con_info_t *ci = con->arch.mxm.ci;
    if (!ci) { return; }
...
static void pscom_mxm_init_con(pscom_con_t *con)
{
...
static void pscom_mxm_init(void)
{
    ...
    /*
      Disabled. Init will be called with the first connect.
            // Preinitialize psmxm. Ignore errors. pscom_mxm_connect will see
      the error again. psmxm_init();
    */
}
...
static int pscom_mxm_con_init(pscom_con_t *con)
{
    return psmxm_init();
}


static void pscom_mxm_handshake(pscom_con_t *con, int type, void *data,
                                unsigned size)
{
    ...
    case PSCOM_INFO_ARCH_REQ: {
        ...
        ret = pscom_precon_send(con->precon, PSCOM_INFO_MXM_ID, &msg,
                                sizeof(msg));
    ...
    case PSCOM_INFO_MXM_ID: {
        ...
        if (psmxm_con_connect(con->arch.mxm.ci, msg, con)) {
    ...
    case PSCOM_INFO_EOF: pscom_mxm_init_con(con); break; /* Done. Use Mxm */
    }
    return;
    /* --- */
error_con_connect:
...
static void pscom_mxm_finalize(void)
{
...
pscom_plugin_t pscom_plugin_mxm = {
    .name          = "mxm",
