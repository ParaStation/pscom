                           ...
                           size_t size, unsigned int magic)
{
    ...
    */
    ...
    rc = rma2_post_put_bt(
        con_info->rma2_port, con_info->rma2_handle, send->bufs.mr,
        (unsigned)((char *)_msg - (char *)send->bufs.ptr), psex_len,
        PSEX_DATA(con_info->remote_rbuf_nla +
                      con_info->remote_recv_pos * sizeof(psex_msg_t),
                  psex_len),
        0, 0);
...
void psex_progress(void)
{
    ...
    if (rc == RMA2_SUCCESS) { rma2_noti_free(rma2_port, notification); }
#endif
}
    ...
    while (1) {
        ...
        if (!magic) { // Nothing received
            *buf = NULL;
            ...
            _psex_send_tokens(con_info);
            // psex_progress(con_info); Called outside the loop over all
            // connections.
