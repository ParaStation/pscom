...
static
unsigned psdapl_page_size = 0;


static
void psdapl_page_size_init(void)
{
	if (!psdapl_page_size) {
		psdapl_page_size = getpagesize();
	}
...
static
void psdapl_mregion_deregister(psdapl_mregion_t *mregion)
{
...
static
void psdapl_mregion_cache_clear(void)
{
	int i;
...
static
void psdapl_mregion_cache_init(void)
{
...
static inline
int psdapl_mregion_is_inside(void *mreg_buf, size_t mreg_size,
...
static
psdapl_mregion_cache_t *psdapl_mregion_find(void *buf, size_t size,
					    psdapl_socket_t *socket)
{
	struct list_head *pos;
	list_for_each(pos, &psdapl_mregion_cache) {
		psdapl_mregion_cache_t *mreg = list_entry(pos, psdapl_mregion_cache_t, next);
...
static
void psdapl_mregion_use(psdapl_mregion_cache_t *mreg)
{
	mreg->use_cnt ++;
...
static
void psdapl_mregion_use_done(psdapl_mregion_cache_t *mreg)
{
	mreg->use_cnt --;
}
...
static
psdapl_mregion_cache_t *psdapl_mregion_get_oldest(void)
{
...
static
DAT_RETURN psdapl_mregion_register(psdapl_mregion_t *mregion,
			    void *buf, size_t size,
			    psdapl_socket_t *socket)
{
	DAT_RETURN dat_rc;
...
psdapl_get_mregion(void *buf, size_t size, psdapl_con_info_t *ci)
{
	...
	DAT_RETURN dat_rc;
	dat_rc = psdapl_mregion_register(&mreg->mregion, buf, size,
					 ci->socket);
...
int psdapl_post_rdma_get(psdapl_rdma_req_t *req)
{
	...
	dat_rc = dat_ep_post_rdma_read(ci->ep_handle, 1,
				       &lmr, cookie, &rmr, 0/* DAT_COMPLETION_SUPPRESS_FLAG*/);
