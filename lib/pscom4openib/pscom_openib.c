...
pscom_openib_env_parser_set_pending_tokens(void *buf, const char *config_val)
{
    const char *set_val = config_val ? config_val
                                     : psoib_pending_tokens_suggestion_str();

    return pscom_env_parser_set_config_uint(buf, set_val);
}


static pscom_err_t pscom_openib_env_parser_set_sendq_size(void *buf,
...
pscom_env_table_entry_t pscom_env_table_openib[] = {
    {"HCA", NULL,
     ...
     "hca).",
     &psoib_hca, PSCOM_ENV_ENTRY_FLAGS_EMPTY, PSCOM_ENV_PARSER_STR},

    {"PORT", "0", "Port to use (default is first active port).", &psoib_port,
     PSCOM_ENV_ENTRY_FLAGS_EMPTY, PSCOM_ENV_PARSER_UINT},

    {"PATH_MTU", "3", "MTU of the IB packets. (1:256, 2:512, 3:1024)",
     &psoib_path_mtu, PSCOM_ENV_ENTRY_FLAGS_EMPTY, PSCOM_ENV_PARSER_UINT},

    {"RECVQ_SIZE", "16", "Number of receive buffers per connection.",
     &psoib_recvq_size, PSCOM_ENV_ENTRY_FLAGS_EMPTY, PSCOM_ENV_PARSER_UINT},

    {"GLOBAL_SENDQ", "0", "Enable/disable global send queue.",
     &psoib_global_sendq, PSCOM_ENV_ENTRY_FLAGS_EMPTY, PSCOM_ENV_PARSER_INT},

    {"COMPQ_SIZE", "128",
     "Size of the completion queue. This likewise corresponds to the size "
     "of the global send queue (if enabled).",
     &psoib_compq_size, PSCOM_ENV_ENTRY_FLAGS_EMPTY, PSCOM_ENV_PARSER_UINT},
    ...
    {"EVENT_CNT", "1",
     ...
     &psoib_event_count, PSCOM_ENV_ENTRY_FLAGS_EMPTY, PSCOM_ENV_PARSER_INT},
    ...
    {"LID_OFFSET", "0", "Offset to base LID (adaptive routing).",
     &psoib_lid_offset, PSCOM_ENV_ENTRY_FLAGS_EMPTY, PSCOM_ENV_PARSER_INT},

    {"RENDEZVOUS", "40000", "The rendezvous threshold for pscom4openib.",
    ...
    {"RNDV_FALLBACKS", "1",
     ...
     &psoib_rndv_fallbacks, PSCOM_ENV_ENTRY_FLAGS_EMPTY, PSCOM_ENV_PARSER_INT},
...
static pscom_poll_t pscom_poll_cq;

__attribute__((visibility("hidden"))) int pscom_do_poll_cq(pscom_poll_t *poll)
{
    psoib_progress();
...
static inline void pscom_check_cq_poll(void)
{
    if (psoib_outstanding_cq_entries) {
...
static int _pscom_openib_do_read(pscom_con_t *con, psoib_con_info_t *mcon)
{
    void *buf;
    ssize_t size;
...
static int pscom_openib_do_read(pscom_poll_t *poll)
{
...
static int pscom_openib_do_write(pscom_poll_t *poll)
{
...
static inline pscom_rendezvous_data_openib_t *
...
static int pscom_openib_rma_mem_register_check(pscom_con_t *con,
...
static unsigned int pscom_openib_rma_mem_register(pscom_con_t *con,
                                                  pscom_rendezvous_data_t *rd)
{
    int err                                   = 0;
    pscom_rendezvous_data_openib_t *openib_rd = get_req_data(rd);
...
#error IB_RNDV_USE_PADDING and IB_RNDV_RDMA_WRITE are mutually exclusive!
    ...
    rd->msg.arch.openib.padding_size = (IB_RNDV_PADDING_SIZE -
                                        ((long long int)rd->msg.data) %
                                            IB_RNDV_PADDING_SIZE) %
                                       IB_RNDV_PADDING_SIZE;

    memcpy(rd->msg.arch.openib.padding_data, rd->msg.data,
           rd->msg.arch.openib.padding_size); // ToDo: _pscom_memcpy?

    /* get mem region */
    perf_add("openib_acquire_rma_mreg");
    err = psoib_acquire_rma_mreg(
        mreg, rd->msg.data + rd->msg.arch.openib.padding_size,
        rd->msg.data_len - rd->msg.arch.openib.padding_size, ci);
    ...
    return sizeof(rd->msg.arch.openib) -
           sizeof(rd->msg.arch.openib.padding_data) +
           rd->msg.arch.openib.padding_size;
#else
...
static void pscom_openib_rma_mem_deregister(pscom_con_t *con,
                                            pscom_rendezvous_data_t *rd)
{
    pscom_rendezvous_data_openib_t *openib_rd = get_req_data(rd);
    psoib_rma_mreg_t *mreg                    = &openib_rd->rma_req.mreg;
...
static void pscom_openib_rma_read_io_done(void *priv, int err)
{
    ...
    if (unlikely(err)) { rendezvous_req->pub.state |= PSCOM_REQ_STATE_ERROR; }
    _pscom_recv_req_done(rendezvous_req);
}


static int pscom_openib_rma_read(pscom_req_t *rendezvous_req,
                                 pscom_rendezvous_data_t *rd)
{
    int err;
...
#ifdef IB_RNDV_USE_PADDING
    memcpy(rendezvous_req->pub.data, rd->msg.arch.openib.padding_data,
           rd->msg.arch.openib.padding_size);
    rendezvous_req->pub.data += rd->msg.arch.openib.padding_size;
    rendezvous_req->pub.data_len -= rd->msg.arch.openib.padding_size;
#endif

    err = psoib_acquire_rma_mreg(&dreq->mreg, rendezvous_req->pub.data,
...
static void pscom_openib_rma_write_io_done(void *priv, int err)
{
...
static int pscom_openib_rma_write(pscom_con_t *con, void *src,
                                  pscom_rendezvous_msg_t *des,
                                  void (*io_done)(void *priv, int err),
                                  void *priv)
{
    pscom_rendezvous_data_t *rd_data = (pscom_rendezvous_data_t *)pscom_malloc(
        sizeof(*rd_data));
    pscom_rendezvous_data_openib_t *rd_data_openib = get_req_data(rd_data);
    psoib_con_info_t *mcon                         = con->arch.openib.mcon;
...
static void pscom_openib_con_cleanup(pscom_con_t *con)
{
    psoib_con_info_t *mcon = con->arch.openib.mcon;
...
static void pscom_openib_con_close(pscom_con_t *con)
{
    psoib_con_info_t *mcon = con->arch.openib.mcon;
...
static void pscom_poll_read_start_openib(pscom_con_t *con)
{
    pscom_poll_read_start(con, pscom_openib_do_read);
}


static void pscom_poll_write_start_openib(pscom_con_t *con)
{
    pscom_poll_write_start(con, pscom_openib_do_write);
}


static void pscom_openib_init_con(pscom_con_t *con)
{
...
/*********************************************************************/
static void pscom_openib_init(void)
{
...
static int pscom_openib_con_init(pscom_con_t *con)
{
    return psoib_init();
}
                                   ...
                                   unsigned size)
{
    ...
    case PSCOM_INFO_OIB_ID: {
        ...
        if (psoib_con_connect(con->arch.openib.mcon, msg)) {
