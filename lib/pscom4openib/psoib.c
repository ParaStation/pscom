...
#define IB_MTU                                                                 \
    (16 * 1024) /* must be < 65536, or change sizeof                           \
                   psoib_msgheader_t.payload, and should be a power of         \
                   IB_MTU_SPEC */
...
#ifdef IB_USE_RNDV
    /* RMA */
    struct list_head rma_reqs; /* list of active RMA requests :
                                  psiob_rma_req_t.next */
#endif
};
...
int psoib_event_count  = 1; /* bool. Be busy if outstanding_cq_entries is to
                               high? */
...
int psoib_rndv_fallbacks = 1; /* bool: Use eager/sw-rndv if memory cannot be
                                 registered for rndv? */
...
static void psoib_err(char *str)
{
...
static void psoib_err_errno(char *str, int err_no)
{
...
static void psoib_scan_hca_ports(struct ibv_device *ib_dev)
{
    struct ibv_context *ctx;
    ...
    psoib_dprint(D_DBG_V, "psoib_scan_hca_ports(\"%s\") port_cnt=%u", dev_name,
                 port_cnt);
    ...
    for (port = 1; port <= port_cnt; port++) {
        ...
        if (port_state == IBV_PORT_ACTIVE &&
            ...
            (!psoib_port || psoib_port == port) && (port_attr.lid != 0)) {
        ...
        psoib_dprint(D_DBG_V, "IB port <%s:%u> lid:%u : %s%s", dev_name, port,
                     port_attr.lid, port_state_str(port_state), marker);
    }
...
static void psoib_scan_all_ports(void)
{
    ...
    // psoib_dprint(D_DBG_V, "configured port <%s>", port_name(psoib_hca,
    // psoib_port));
...
static struct ibv_device *psoib_get_dev_by_hca_name(const char *in_hca_name)
{
...
static struct ibv_context *psoib_open_hca(char *hca_name)
{
...
static struct ibv_cq *psoib_open_cq(struct ibv_context *ctx, int cqe_num)
{
    ...
    if (!cq) { psoib_err_errno("ibv_create_cq() failed", errno); }

    return cq;
}

static struct ibv_pd *psoib_open_pd(struct ibv_context *ctx)
{
    ...
    if (!pd) { psoib_err_errno("ibv_alloc_pd() failed", errno); }

    return pd;
}


static void psoib_vapi_free(psoib_hca_info_t *hca_info, mem_info_t *mem_info)
{
...
static void print_mlock_help(size_t size)
{
    static int called = 0;
    struct rlimit rlim;
    ...
    psoib_dprint(D_WARNONCE, "(Check memlock limit in "
                             "/etc/security/limits.conf or try 'ulimit -l')");

    if (!getrlimit(RLIMIT_MEMLOCK, &rlim)) {
        psoib_dprint(D_WARNONCE,
                     "Current RLIMIT_MEMLOCK: soft=%lu byte, hard=%lu byte",
                     rlim.rlim_cur, rlim.rlim_max);
    }
}

static int psoib_vapi_alloc(psoib_hca_info_t *hca_info, int size,
                            ...
                            mem_info_t *mem_info)
{
    ...
    //    printf("ibv_reg_mr(pd = %p, ptr = %p, size = %d, access_perm =
    //    0x%x)\n",
 ...
 *  move_to_rtr
 */
static int move_to_rtr(struct ibv_qp *qp, uint8_t port_num,
                       ...
                       uint32_t remote_qpn) /* remote peer's QPN */
{
    struct ibv_qp_attr attr = {.qp_state    = IBV_QPS_RTR,
                               ...
                               .max_dest_rd_atomic = 1, /* Maximum number of
                                                           oust. RDMA
                                                           read/atomic as target
                                                         */
                               .min_rnr_timer = 12,     /* Minimum RNR NAK timer
                                                           (old = 0) */
                               .ah_attr = {.is_global = 0, /* old av.grh_flag ?
                                                            */
                                           .dlid      = remote_lid,
 ...
 *  move_to_rts
 */
static int move_to_rts(struct ibv_qp *qp)
{
    struct ibv_qp_attr attr = {
        ...
        .max_rd_atomic = 1,  /* Number of outstanding RDMA rd/atomic ops at
                                destination ?*/
    };
                   ...
                   port_info_t *port_info)
{
            ...
            .cap =
                {
                    ...
                    //.max_send_sge = 4,   /* Max scatter/gather descriptor
                    // entries on the SQ ??*/
                    //.max_recv_sge = 4,   /* Max scatter/gather descriptor
                    // entries on the RQ */
                    .max_send_wr  = 128, /* Max outstanding WR on the SQ ??*/
                    .max_recv_wr  = 128, /* Max outstanding WR on the RQ ??*/
                    .max_send_sge = 1, /* Max scatter/gather descriptor entries
                                          on the SQ ??*/
                    .max_recv_sge = 1, /* Max scatter/gather descriptor entries
                                          on the RQ */
    ...
    if (!psoib_global_sendq) {
        if (psoib_vapi_alloc(hca_info, IB_MTU * psoib_sendq_size, 0,
                             &con_info->send.bufs)) {
            goto err_alloc;
        }
    }
...
int psoib_con_connect(psoib_con_info_t *con_info, psoib_info_msg_t *info_msg)
{
    ...
    if (move_to_rtr(con_info->qp, con_info->port_info->port_num, info_msg->lid,
                    info_msg->qp_num)) {
        goto err_move_to_rtr;
    }

    if (move_to_rts(con_info->qp)) { goto err_move_to_rts; }

    // Initialize send tokens
    con_info->n_send_toks = psoib_recvq_size; // #tokens = length of _receive_
                                              // queue!
...
static void psoib_cleanup_hca(psoib_hca_info_t *hca_info)
{
...
static int psoib_init_hca(psoib_hca_info_t *hca_info)
{
    struct ibv_device_attr device_attr;
    ...
    if (psoib_pending_tokens > psoib_recvq_size) {
        psoib_dprint(D_WARN,
                     "warning: reset psoib_pending_tokens from %u to %u\n",
...
static int psoib_init_port(psoib_hca_info_t *hca_info, port_info_t *port_info)
{
...
int psoib_init(void)
{
    ...
    if (init_state == 1) {
        ...
        if (psoib_init_port(&psoib_default_hca, &default_port)) {
            goto err_port;
        }
        init_state = 0;
    }
...
static int psoib_poll(psoib_hca_info_t *hca_info, int blocking);
...
static inline int _psoib_sendv(psoib_con_info_t *con_info, struct iovec *iov,
                               size_t size, unsigned int magic)
{
    ...
    */
    ...
    if (con_info->outstanding_cq_entries >= psoib_sendq_size &&
        psoib_event_count) {
        // printf("Busy local\n"); usleep(10*1000);
...

    {
        ...
        struct ibv_send_wr wr = {
            ...
            .send_flags = ((ENABLE_SEND_NOTIFICATION ? IBV_SEND_SIGNALED
                                                     : 0) | /* no cq
                                                               entry,
                                                               if
                                                               unsignaled
                                                             */
                           ((list.length <= IB_MAX_INLINE) ? IBV_SEND_INLINE
                                                           : 0)),
            .imm_data   = 42117,
            ...
            .wr.rdma =
                {
                    .remote_addr = (uint64_t)
                        PSOIB_DATA((((psoib_msg_t *)con_info->remote_ptr) +
...
static void _psoib_send_tokens(psoib_con_info_t *con_info)
{
...
int psoib_recvlook(psoib_con_info_t *con_info, void **buf)
{
    ...
    while (1) {
        psoib_msg_t *msg = ((psoib_msg_t *)con_info->recv.bufs.ptr) +
                           con_info->recv.pos;
    ...
    while (con_info->n_recv_toks > 0) {
        psoib_msg_t *msg = ((psoib_msg_t *)con_info->recv_bufs.ptr) +
                           con_info->recv_pos;
...
static void psoib_all_con_broken(psoib_hca_info_t *hca_info)
{
...
static int psoib_check_cq(psoib_hca_info_t *hca_info)
{
    struct ibv_wc wc;
    int rc;
        ...
        if (wc.opcode == IBV_WC_RDMA_WRITE /* == VAPI_CQE_SQ_RDMA_WRITE ?*/) {
            ...
            if (con->magic == MAGIC_PSOIB_CONNECTION) {
                ...
                if (wc.status == IBV_WC_SUCCESS) {
                    //		    printf("RDMA write done... recv: %d tosend:
                    //%d send: %d\n", 			   con->n_recv_toks,
                    // con->n_tosend_toks, con->n_send_toks);
                    ;
                } else {
                    psoib_dprint(D_ERR,
                                 "Failed RDMA write request (status %d : %s). "
                                 "Connection broken!",
                ...
                psoib_rma_req_t *dreq = (psoib_rma_req_t *)(unsigned long)
                                            wc.wr_id;
...
//		printf("RDMA write done...\n");
		...
		if (!fail_emulate_countdown) failed = 1; // fake fail.
            ...
            if (failed) {
                psoib_dprint(D_ERR,
                             "Failed RDMA READ request (status %d : %s). "
                             "Connection broken!",
                             wc.status, ibv_wc_status_str(wc.status));
                req->ci->con_broken = 1;
            }
            ...
            req->io_done(req, failed);
                   ...
                   (IBV_WC_SEND | IBV_WC_RECV) /* == VAPI_CQE_RQ_SEND_DATA*/) {
            /* receive something */
            psoib_con_info_t *con = (psoib_con_info_t *)(unsigned long)wc.wr_id;
            ...
            if (wc.status == IBV_WC_SUCCESS) {
                ...
                psoib_dprint(D_ERR,
                             "Failed receive request (status %d : %s). "
                             "Connection broken!",
                             wc.status, ibv_wc_status_str(wc.status));
                con->con_broken = 1;
            }
            ...
            if (wc.status == IBV_WC_SUCCESS) {
                ...
                psoib_dprint(D_ERR,
                             "Failed send request (status %d : %s). Connection "
                             "broken!",
                             wc.status, ibv_wc_status_str(wc.status));
                con->con_broken = 1;
            }
            ...
            psoib_dprint(psoib_ignore_wrong_opcodes ? D_WARN : D_ERR,
                         "ibv_poll_cq(): Infiniband returned the wrong Opcode "
                         "%d",
                         wc.opcode);
            if (!psoib_ignore_wrong_opcodes) {
                openlog(NULL, LOG_PID, LOG_USER);
                syslog(LOG_ERR,
                       "pscom4openib: ibv_poll_cq(): Infiniband returned the "
                       "wrong Opcode %d",
                       wc.opcode);
...
static int psoib_rma_mreg_register(psoib_rma_mreg_t *mreg, void *buf,
                                   size_t size, psoib_con_info_t *ci)
{
...
static int psoib_rma_mreg_deregister(psoib_rma_mreg_t *mreg)
{
    int ret;
    mem_info_t *mem_info = &mreg->mem_info;
...
static void psoib_rma_reqs_enq(psoib_rma_req_t *req)
{
