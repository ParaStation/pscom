...
static const unsigned pscom_async_msg_sizes[] = {
...
static pscom_async_ipc_t pscom_async_ipc = {
    .running = 0,
};


static struct list_head pscom_timerq = LIST_HEAD_INIT(pscom_timerq);
...
static void pscom_backlog_getall(struct list_head *backlog)
{
    pthread_mutex_lock(&pscom.backlog_lock);
    {
...
static void pscom_async_msg_attach(pscom_async_ipc_t *ipc, int fd,
                                   async_cb_t async_cb, void *priv)
{
...
static void pscom_async_msg_detach(pscom_async_ipc_t *ipc, int fd,
                                   async_cb_t async_cb, void *priv)
{
    ufd_info_t *ufd_info;
...
static void pscom_timer_add(pscom_timer_t *timer)
{
...
static void pscom_timer_del(pscom_timer_t *timer)
{
    list_del(&timer->next);
}


// Execute all expired timers. Return msec remaining to the next timer or -1 if
// none.
static int pscom_timer_exec(void)
{
    struct list_head *pos, *next;
    ...
    pscom.ufd_timeout = timeout; // ToDo: Fix potential race on
                                 // pscom.ufd_timeout
    return timeout;
}


static void pscom_async_msg_timer(pscom_async_ipc_t *ipc, unsigned msec,
                                  pscom_timer_cb_t timer_cb, void *priv)
{
...
static void pscom_async_thread_service_read(ufd_t *ufd, ufd_info_t *ufd_service)
{
    ...
    msg_rest_len = pscom_async_msg_sizes[msg.msg_common.msg_type] -
                   (unsigned)sizeof(msg.msg_common);
    ...
    case PSCOM_ASYNC_MSG_NONE: break;
    case PSCOM_ASYNC_MSG_ATTACH:
        pscom_async_msg_attach(ipc, msg.msg_attach.fd, msg.msg_attach.async_cb,
                               msg.msg_attach.priv);
        break;
    case PSCOM_ASYNC_MSG_DETACH:
        pscom_async_msg_detach(ipc, msg.msg_detach.fd, msg.msg_detach.async_cb,
                               msg.msg_detach.priv);
        break;
    case PSCOM_ASYNC_MSG_TIMER:
        pscom_async_msg_timer(ipc, msg.msg_timer.msec, msg.msg_timer.timer_cb,
                              msg.msg_timer.priv);
        break;
    }
...
static void pscom_async_client_send(pscom_async_msg_t *msg)
{
    int rlen;
    ...
    // Busy wait for ack
    while (!ack) { sched_yield(); }
}


void pscom_async_on_readable(int fd, async_cb_t *async_cb, void *priv)
{
    pscom_async_client_send(
        &(pscom_async_msg_t){.msg_attach = {.msg_type = PSCOM_ASYNC_MSG_ATTACH,
                                            .fd       = fd,
                                            .async_cb = async_cb,
                                            .priv     = priv}});
}
...
{
    pscom_async_client_send(
        &(pscom_async_msg_t){.msg_detach = {.msg_type = PSCOM_ASYNC_MSG_DETACH,
                                            .fd       = fd,
                                            .async_cb = async_cb,
                                            .priv     = priv}});
}
...
{
    if ((pscom.ufd_timeout == -1) || ((unsigned)pscom.ufd_timeout > msec)) {
        pscom.ufd_timeout = msec;
    }
    pscom_async_client_send(
        &(pscom_async_msg_t){.msg_timer = {.msg_type = PSCOM_ASYNC_MSG_TIMER,
                                           .msec     = msec,
                                           .timer_cb = timer_cb,
                                           .priv     = priv}});
}
