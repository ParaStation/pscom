 ...
 */
static
const unsigned bcast_devide = 3; // ToDo: make it configurable/ messagelen dependant
...
static inline
unsigned bcast_rank_iter_begin(bcast_rank_iter_t *iter,
...
static inline
unsigned bcast_rank_iter_end(bcast_rank_iter_t *iter)
{
...
static inline
unsigned bcast_rank_iter_next(bcast_rank_iter_t *iter, unsigned rank)
{
...
static
void _gcompat_init(unsigned recvs[],
		   unsigned my_rank,
		   unsigned group_size, unsigned devide)
{
...
static
pscom_con_t *_pscom_get_bcast_recv_con(pscom_group_t *group, unsigned bcast_root)
{
	unsigned my_urank = (group->my_grank + group->group_size - bcast_root) % group->group_size;
	unsigned urank_recv_from = group->compat[my_urank];
	unsigned rank_recv_from = (urank_recv_from + bcast_root) % group->group_size;
...
static
void _replay_bcast_req(pscom_req_t *req_bcast)
{
	pscom_req_t *req_master = _pscom_get_bcast_receiver(get_con(req_bcast->pub.connection),
							    &req_bcast->pub.header);
	if (!req_master) return; // error

	pscom_req_prepare_recv(req_master, &req_bcast->pub.header, req_bcast->pub.connection);
	pscom_req_write(req_master, req_bcast->pub.data, req_bcast->pub.data_len);

	_pscom_update_recv_req(req_master); // io_done will call _bcast_req_master_step()
}


static
void io_done_replay_bcast_req(pscom_request_t *request)
{
...
static
void _pscom_req_send_continue(pscom_req_t *req)
{
...
static void _bcast_req_master_step(pscom_req_t *req_master)
{
	...
	// req_recv_user done? (check after fw sends in progress (maybe shared data pointer!!!))
	...
	if (!(req_master->pub.state & PSCOM_REQ_STATE_DONE /*PSCOM_REQ_STATE_IO_DONE*/)) return;
...
static
void _bcast_req_master_write(pscom_req_t *req_master, char *buf, unsigned len)
{
	pscom_bcast_req_t *bc_master = (pscom_bcast_req_t *)req_master->pub.user;
	...
	list_for_each(pos, &bc_master->fw_send_requests) {
		...
		// printf("%s:%d Forward send request: ->con %p\n", __func__, __LINE__, req_send->pub.connection); // ToDo:
...
static
void io_done_bcast_req_master(pscom_request_t *request)
{
...
static
void check_recv_accept(pscom_req_t *req)
{
	static int warned = 0;
...
static
void _recvq_enq(pscom_con_t *con, pscom_group_mem_t *mem, pscom_req_t *req)
{
...
static
void _recvq_deq(pscom_req_t *req)
{
...
static
pscom_req_t *_recvq_find_and_deq(pscom_group_t *group, pscom_group_mem_t *mem, pscom_header_net_t *nh)
{
...
static
pscom_req_t *_find_genrecv_req(pscom_group_t *group, pscom_group_mem_t *mem, pscom_req_t *req)
{
	check_recv_accept(req);
	...
	if (!list_empty(&mem->genrecvq)) {
		...
		// return first list entry
		pscom_req_t *req_master = list_entry(mem->genrecvq.next, pscom_req_t, next_alt);
		return req_master;
	}
	return NULL;
}
...
static
pscom_req_t *generate_fwrecv_req(pscom_group_t *group,
				 unsigned xheader_len, unsigned data_len,
				 pscom_req_t *req_recv_user, pscom_req_t *req_send_user)
{
	...
	if (req_recv_user) {
		...
		req_master = pscom_req_create(xheader_len, sizeof(pscom_bcast_req_t) + data_len);
		bc_master = (pscom_bcast_req_t *)req_master->pub.user;
		data = bc_master->data;
		pscom.stat.gen_reqs++;
	}
	...
	bc_master->req_recv_user_done = !!req_send_user; // in send mode fake a recv_user_done
...
static
void _pscom_post_bcast_receive(pscom_req_t *req_user, pscom_group_t *group, unsigned bcast_root)
{
	...
	if (!req_master) {
		req_master = generate_fwrecv_req(group, req_user->pub.xheader_len, req_user->pub.data_len,
						 req_user, NULL);
		...
		// Copy header
		pscom_req_prepare_recv(req_user, &req_master->pub.header, req_master->pub.connection);

		// Copy data
		pscom_req_write(req_user, req_master->pub.data,
				(char*)req_master->cur_data.iov_base - (char *)req_master->pub.data);
...
static
pscom_req_t *fw_send_create(char *data_master, pscom_header_net_t *nh)
{
...
static
void init_send_forwards(pscom_group_t *group, pscom_req_t *req_master, pscom_header_net_t *nh)
{
	pscom_bcast_req_t *bc_master = (pscom_bcast_req_t *)req_master->pub.user;
	...
	for (urank = bcast_rank_iter_begin(&iter, subg_size - 1, bcast_devide);
		...
		req_send->pub.connection = group_rank2connection(group, dest); // destination
		...
		pscom_req_prepare_send_pending(req_send, PSCOM_MSGTYPE_BCAST, nh->data_len);
		// ToDo: Use pscom_post_send_rendezvous() for long messages?
		// called in _bcast_req_master_write(): pscom_post_send_direct(req, PSCOM_MSGTYPE_BCAST);
	}
}
...
pscom_req_t *_pscom_get_bcast_receiver(pscom_con_t *con, pscom_header_net_t *nh)
{
	...
	pscom_group_t *group = _pscom_group_find(get_sock(con->pub.socket), group_id);
	...
	// ToDo: disable assert (check if we receive from the expected group member):
...
unknown_group:
	...
	list_add_tail(&req_gen->next, &(get_sock(con->pub.socket)->group_req_unknown));
...
static
void _pscom_post_bcast_send(pscom_req_t *req_user, pscom_group_t *group)
{
	assert(req_user->pub.xheader_len >= sizeof(pscom_xheader_bcast_t));
	...
	req_user->pub.state = PSCOM_REQ_STATE_SEND_REQUEST | PSCOM_REQ_STATE_POSTED |
	...
	req_master->pub.state = PSCOM_REQ_STATE_SEND_REQUEST | PSCOM_REQ_STATE_POSTED | PSCOM_REQ_STATE_DONE;
	_bcast_req_master_write(req_master, req_user->pub.data, req_user->pub.data_len);
}
				    ...
				    void *data, size_t data_len)
{
	...
	if (xheader) {
		memcpy(req->pub.xheader.bcast.user, xheader, xheader_len);
	}
...
static
int recv_accept_bcast(pscom_request_t *request,
		      pscom_connection_t *connection,
		      pscom_header_net_t *header_net)
{
		 ...
		 void *data, size_t data_len)
{
	...
	if (bcast_root != group->my_grank) {
		...
		req = pscom_bcast_create_req(group,
					     NULL, xheader_len,
					     data, data_len);
		...
		if (xheader_len) memcpy(xheader, req->pub.xheader.bcast.user, xheader_len);
...
void pscom_post_bcast(pscom_request_t *request)
{
	...
	pscom_bcast(group, bcast_root,
		    request->xheader.bcast.user, request->xheader_len - sizeof(req->pub.xheader.bcast),
