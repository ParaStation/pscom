...
static void _write_start_suspending(pscom_con_t *con)
{
    struct list_head *pos, *next;
    int suspend_found = 0;

    // Move all send requests posted after PSCOM_MSGTYPE_SUSPEND to
    // sendq_suspending. If there is no PSCOM_MSGTYPE_SUSPEND, it is already
    // sent and all reqs will be moved.
    while (1) {
        list_for_each_safe (pos, next, &con->sendq) {
            ...
            if (req->pub.header.msg_type == PSCOM_MSGTYPE_SUSPEND) {
                ...
                _pscom_sendq_suspending_enq(con, req); // enqueue to suspend
                                                       // queue
            }
        }
        if (suspend_found) { break; }
...
static void suspend_init_con(pscom_con_t *con)
{
...
static void _pscom_con_check_suspended(pscom_con_t *con)
{
    if ((con->pub.state & PSCOM_CON_STATE_SUSPENDED) !=
        PSCOM_CON_STATE_SUSPENDED) {
        return;
    }
...
static void io_done_send_suspend(pscom_req_state_t state, void *priv_con)
{
...
static void _pscom_con_send_suspend(pscom_con_t *con, int portno)
{
    _pscom_send_inplace(con, PSCOM_MSGTYPE_SUSPEND, &portno, sizeof(portno),
                        NULL, 0, io_done_send_suspend, con);
}


static void _pscom_con_send_resume(pscom_con_t *con)
{
    _pscom_send_inplace(con, PSCOM_MSGTYPE_SUSPEND, NULL, 0, NULL, 0, NULL,
                        NULL);
}
...
void _pscom_con_suspend(pscom_con_t *con)
{
    ...
    if (con->pub.type == PSCOM_CON_TYPE_LOOP) {
        return; // Nothing to do
    }
    if ((con->pub.state & PSCOM_CON_STATE_SUSPENDING) != 0) {
        return; // Already called
    }
    if ((con->pub.state & PSCOM_CON_STATE_W) == 0) {
        return; // Can't write
    }
    ...
    if (err == PSCOM_ERR_ALREADY) { pscom_listener_active_inc(&sock->listen); }
    ...
    // Listen for PSCOM_MSGTYPE_SUSPEND in response (see
    // _pscom_con_suspend_received() for dec(con))
                                 ...
                                 size_t xheaderlen)
{
    ...
    DPRINT(D_SUSPEND_DBG, "SUSPEND received on %s, port %d",
           pscom_con_str(&con->pub), portno);
    ...
    // Stop listening for PSCOM_MSGTYPE_SUSPEND (see _pscom_con_suspend() for
    // inc(con))
