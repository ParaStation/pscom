...
{
    char buf[80];
    snprintf(buf, sizeof(buf), "con/%.8s/%s", con->pub.remote_con_info.name,
             path);
...
static void _pscom_con_terminate_sendq(pscom_con_t *con)
{
    ...
    list_for_each_safe (pos, next, &pscom.requests) {
        ...
        if (req->pub.connection == &con->pub) {
            ...
            if ((mask_rndv_send_posted & req->pub.state) ==
                mask_rndv_send_posted) {
                // Rendezvous send requests are waiting for an ACK message. If
                // the connection dies, the ACK will never arrive. Therefor we
                // decrement the pending counter here.
                if (!(req->pub.state & PSCOM_REQ_STATE_IO_DONE)) {
                    _pscom_write_pendingio_cnt_dec(
                        con, req); // inc in
                                   // pscom_prepare_send_rendezvous_inline()
                    _pscom_send_req_done(req); // done with error (error flag
                                               // set in _pscom_pendingio_abort)
                }
            }
        }
    }

    // Connection suspending? Terminate send requests from
    // sock->sendq_suspending.
...
static inline void _pscom_recv_req_terminate_deq(pscom_req_t *req)
{
...
static void _pscom_con_terminate_net_queues(pscom_con_t *con)
{
    // genreqq:
    while (!list_empty(&con->net_recvq_user)) {
        pscom_req_t *req = list_entry(con->net_recvq_user.next, pscom_req_t,
                                      next);
...
static void pscom_con_end_read(pscom_con_t *con)
{
    con->read_stop(con);
...
static void pscom_con_end_write(pscom_con_t *con)
{
    con->write_stop(con);
...
static void pscom_con_close_write(pscom_con_t *con)
{
    con->pub.state &= ~PSCOM_CON_STATE_W; // clear W
...
static void pscom_con_error_read_failed(pscom_con_t *con, pscom_err_t error)
{
...
static void pscom_con_error_write_failed(pscom_con_t *con, pscom_err_t error)
{
...
static void _pscom_con_cleanup(pscom_con_t *con)
{
    assert(con->magic == MAGIC_CONNECTION);
    if (con->state.con_cleanup) {
    ...
    if (con->pub.state != PSCOM_CON_STATE_CLOSED) {
        D_TR(printf("%s:%u:%s(con:%p) : state: %s\n", __FILE__, __LINE__,
                    __func__, con, pscom_con_state_str(con->pub.state)));

        if (!list_empty(&con->net_recvq_user) && con->state.close_called) {
            // This case shouldn't be possible with a valid MPI program not
            // aborting abnormally but with MPI_Finalize() due to the rules of
            // the standard. Nevertheless, in an error case and/or when calling
            // MPI_Abort(), we may very well have the situation where unexpected
            // messages still arrive after actively closing the connection.
    ...
    retry:
        ...
        // At this point, the list of unexpected messages must be empty if the
        // connection was actively closed. (See also the related comment above
        // the retry label.)
        assert(list_empty(&con->net_recvq_user) || !con->state.close_called);

        if (!list_empty(&con->sendq) || !list_empty(&con->recvq_user) ||
        ...
        if (con->pub.state == PSCOM_CON_STATE_CLOSE_WAIT) {
            DPRINT(con->pub.type != PSCOM_CON_TYPE_ONDEMAND ? D_INFO : D_DBG,
                   "DISCONNECT %s via %s", pscom_con_str(&con->pub),
                   pscom_con_type_str(con->pub.type));
        } else {
            DPRINT(D_DBG, "cleanup    %s via %s : %s", pscom_con_str(&con->pub),
                   pscom_con_type_str(con->pub.type),
...
static void io_done_send_eof(pscom_req_state_t state, void *priv_con)
{
...
static void pscom_con_send_eof(pscom_con_t *con)
{
    assert(con->magic == MAGIC_CONNECTION);
    DPRINT(D_DBG_V, "EOF send   %s via %s", pscom_con_str(&con->pub),
           pscom_con_type_str(con->pub.type));

    _pscom_con_ref_hold(con);
    _pscom_send_inplace(con, PSCOM_MSGTYPE_EOF, NULL, 0, NULL, 0,
                        io_done_send_eof, con);
}


static void pscom_con_recv_eof_start_check(pscom_con_t *con)
{
...
static void pscom_con_recv_eof_stop_check(pscom_con_t *con)
{
    if (con->state.eof_expect) {
...
void pscom_con_closing(pscom_con_t *con)
{
    ...
    if (((con->pub.state & PSCOM_CON_STATE_W) == PSCOM_CON_STATE_W)) {
        ...
        default:
            DPRINT(D_WARN,
                   "pscom_con_closing() : unexpected connection state : %s "
                   "(%s)",
                   pscom_con_state_str(con->pub.state),
                   pscom_con_type_str(con->pub.type));
        ...
        case PSCOM_CON_STATE_CLOSE_WAIT:
            ...
            break;
        case PSCOM_CON_STATE_CLOSED: break;
        }
    }
}
...
void pscom_con_close(pscom_con_t *con)
{
    ...
    if (con->con_guard.fd != -1) { pscom_con_guard_stop(con); }
...
static void pscom_con_error_io_done(pscom_request_t *request)
{
    ...
    if (request->socket->ops.con_error && !con->state.close_called) {
        // Call socket->ops.con_error hook if pscom_close_connection() is not
        // yet called.
        request->socket->ops.con_error(request->connection, rdata->operation,
                                       rdata->error);
    }
...
static void pscom_con_error_deferred(pscom_con_t *con, pscom_op_t operation,
                                     pscom_err_t error)
{
    pscom_req_t *req;
...
{
    assert(con->magic == MAGIC_CONNECTION);
    DPRINT((error != PSCOM_ERR_EOF) // EOF is mostly ok (D_DBG), except when
                                    // receiving...
                   || (con->pub.type == PSCOM_CON_TYPE_NONE) // ...EOF while
                                                             // still
                                                             // handshaking (No
                                                             // plugin choosen)
                                                             // is an D_ERR.
           ...
           pscom_con_type_str(con->pub.type),
           pscom_con_state_str(con->pub.state), pscom_op_str(operation),
           pscom_err_str(error));

    _pscom_step();

    switch (operation) {
    case PSCOM_OP_READ: pscom_con_error_read_failed(con, error); break;
    case PSCOM_OP_WRITE: pscom_con_error_write_failed(con, error); break;
...
static void pscom_con_id_increase(void)
{
...
static pscom_con_id_t pscom_con_next_id(void)
{
    pscom_con_id_t id;

    while (1) {
        for (id = pscom_con_id_last + 1;
             (id & pscom_con_ids_mask) !=
             (pscom_con_id_last & pscom_con_ids_mask);
             id++) {
...
static pscom_con_id_t pscom_con_id_register(pscom_con_t *con)
{
    assert(!pscom_con_ids_mask ||
...
static void pscom_con_id_unregister(pscom_con_t *con)
{
    unsigned i;
    if ((!pscom_con_ids_mask) ||
        (pscom_con_ids[con->id & pscom_con_ids_mask] != con)) {
        pscom_con_t *con_reg = pscom_con_ids_mask
                                   ? pscom_con_ids[con->id & pscom_con_ids_mask]
                                   : NULL;
        DPRINT(D_WARN,
               "warning: pscom_con_id_unregister(con:%p [con_reg:%p]) called "
               "more than once!",
               con, con_reg);
...
static void _pscom_con_destroy(pscom_con_t *con)
{
    ...
    if (con->in.readahead.iov_base) { free(con->in.readahead.iov_base); }
...
void pscom_con_setup(pscom_con_t *con)
{
    ...
    pscom_sock_t *sock = get_sock(con->pub.socket);
    if (sock->recv_req_cnt_any) { _pscom_recv_req_cnt_inc(con); }


    if (!list_empty(&con->sendq)) { con->write_start(con); }
    _pscom_step();
}


static int pscom_is_valid_con(pscom_con_t *con)
{
...
void pscom_con_setup_ok(pscom_con_t *con)
{
    ...
    case PSCOM_CON_STATE_ACCEPTING:
        ...
        con->state.internal_connection = 0; // Now the user has to call
                                            // pscom_close_connection() on con.
    ...
    default:
        DPRINT(D_BUG,
               "pscom_con_setup_ok() : connection in wrong state : %s (%s)",
...
static void loopback_write_start(pscom_con_t *con)
{
    int i;
    size_t len;
    struct iovec iov[2];
...
static void pscom_con_guard_error(void *_con)
{
...
static void pscom_guard_readable(ufd_t *ufd, ufd_info_t *ufd_info)
{
    ...
    DPRINT(error ? D_ERR : D_DBG, "pscom guard con:%p %s", con,
...
void pscom_con_guard_stop(pscom_con_t *con)
{
    ...
    if (fd != -1) {
        char msg = GUARD_BYE;
        (void)(write(con->con_guard.fd, &msg, 1) || 0); // Send "Good bye",
                                                        // ignore result
...
pscom_err_t pscom_connect_direct(pscom_con_t *con)
{
    ...
    if (!rc) {
        while (con->pub.type == PSCOM_CON_TYPE_NONE &&
...
static int pscom_connection_is_closing(pscom_connection_t *connection)
{
    switch (connection->state) {
    case PSCOM_CON_STATE_CLOSED:
    case PSCOM_CON_STATE_CLOSE_WAIT:
    case PSCOM_CON_STATE_CLOSING: return 1;
    default: return 0;
    }
}
