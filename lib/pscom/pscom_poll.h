...
// Start polling on poll. It is safe to call pscom_poll_start() multiple times on the same poll.
// pscom_poll_init(poll) must be called before usage with start/stop
void pscom_poll_start(pscom_poll_t *poll, pscom_poll_func_t *do_poll, pscom_poll_list_t *poll_list);


// Stop polling on poll. It is safe to call pscom_poll_stop() multiple times on the same poll.
// Afterwards poll is still en-queued in pscom_poll_list_t, but marked for de-queuing in
// pscom_poll().
void pscom_poll_stop(pscom_poll_t *poll);

// Stop polling on poll and de-queue it from pscom_poll_list_t. You must not call pscom_poll_dequeue() while
// iterating via pscom_poll() over pscom_poll_list_t. Use pscom_poll_stop() instead.
