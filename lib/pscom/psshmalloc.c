...
static struct Psshm_config psshm_config = {
    .min_size = 32UL * 1024 * 1024 /* 32MiB */,
    .max_size = 64UL * 1024 * 1024 * 1024, /* 64 GiB */

};

/* Initialize base pointer with a shared mem segment. Return 0 on success, -1
 * else */
static int psshm_init_base(void)
{
    int shmid;
    void *buf;
    ...
    while (1) {
        shmid = shmget(/*key*/ 0, size,
                       /*SHM_HUGETLB |*/ SHM_NORESERVE | IPC_CREAT | 0777);
        if (shmid != -1) {
            break; // success with size bytes
        }
        if (errno != ENOSPC && errno != EINVAL) {
            goto err; // error, but not "No space left on device" or EINVAL
        }
...
static void *psshm_morecore(ptrdiff_t increment)
{
...
static void getenv_ulong(unsigned long *val, const char *name)
{
    char *aval;
    aval = getenv(name);
    if (aval) { *val = atol(aval); }
}
...
void psshm_init(void)
{
    ...
    //	mallopt(M_MMAP_THRESHOLD, 0/* psshm_config.max_size*/); // always use
    // our psshm_morecore()
...
err_init_base : {
    ...
    snprintf(msg, sizeof(msg),
             ...
             "shmmax\" to "
             "small?)",
             psshm_config.min_size, psshm_config.max_size, strerror(errno));
    psshm_info.msg = msg;
}
