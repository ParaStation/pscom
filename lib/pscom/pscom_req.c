...
static inline void announce_new_req(pscom_req_t *req)
{
    ...
    /* all_req_next is shared with _pscom_pendingio_enq and
     * _pscom_pendingio_enq! */
...
static inline size_t round_up8(size_t val)
{
    return (val + 7) & ~7;
}
...
static struct list_head mallocs = LIST_HEAD_INIT(mallocs);
...
inline void pscom_mverify(void *ptr)
{
...
{
    pscom_req_t *req;
    size_t max_xhl       = pscom_max(round_up8(max_xheader_len),
                                     sizeof(req->pub.xheader));
...
size_t pscom_req_forward(pscom_req_t *req, size_t len)
{
    ...
    void (*write_hook)(pscom_req_t *req, char *buf,
                       size_t len) = req->write_hook;
    if (unlikely(write_hook != NULL)) { req->write_hook = NULL; }

    // pscom_req_write() without _pscom_memcpy_to_user (buf ==
    // req->cur_data.iov_base)
...
void pscom_req_append(pscom_req_t *req, char *buf, size_t len)
{
    ...
    if (buf && buf != tail) { _pscom_memcpy_to_user(tail, buf, len); }
    req->cur_data.iov_len += len;
    req->skip -= len;
}
