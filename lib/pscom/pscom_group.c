...
/***********************/
static void _pscom_group_close(pscom_group_t *group)
{
...
static pscom_group_t *pscom_group_create(pscom_socket_t *socket,
...
static void _pscom_group_open(pscom_group_t *group)
{
...
static inline pscom_req_t *pscom_barrier_create_req(pscom_group_t *group)
{
    pscom_req_t *req;
...
static int recv_accept_barrier(pscom_request_t *request,
...
void pscom_barrier(pscom_group_t *group)
{
    ...
    for (offset = 1; offset < group->group_size; offset = offset * 2) {
        ...
        unsigned src  = (group->my_grank + group->group_size - offset) %
                       group->group_size;
