...
static inline
void pscom_header_net_prepare(pscom_header_net_t *header, pscom_msgtype_t msg_type, size_t xheader_len, size_t data_len)
{
...
{
	pscom_header_net_prepare(&req->pub.header, msg_type, req->pub.xheader_len, req->pub.data_len);
...
static
void _pscom_rma_req_deregister(pscom_con_t *con, pscom_req_t *rma_read_req)
{
	if (con->rma_mem_deregister && rma_read_req->rndv_data) {
		pscom_rendezvous_data_t *rd = (pscom_rendezvous_data_t *) rma_read_req->rndv_data;
...
pscom_req_t *_pscom_get_rma_read_answer_receiver(pscom_con_t *con, pscom_header_net_t *nh)
{
	...
	D_TR(printf("%s:%u:%s(%s)\n", __FILE__, __LINE__, __func__, pscom_debug_req_str(rma_read_req)));
	return rma_read_req;
}
...
pscom_req_t *pscom_get_rendezvous_receiver(pscom_con_t *con, pscom_header_net_t *nh)
{
	...
	pscom_rendezvous_xheader_t *rx = (pscom_rendezvous_xheader_t *)(void *)&nh->xheader;

	size_t user_xheader_len = rx->user_header_net.xheader_len;
	pscom_rendezvous_msg_t *rmsg = (pscom_rendezvous_msg_t *)(rx->user_xheader + user_xheader_len);
	...
	size_t arch_len = pscom_rendezvous_arch_len(nh->xheader_len, user_xheader_len);
	...
	D_TR(printf("%s:%u:%s(%s)\n", __FILE__, __LINE__, __func__, pscom_debug_req_str(rndv_req)));
...
pscom_req_t *_pscom_get_rendezvous_fin_receiver(pscom_con_t *con, pscom_header_net_t *nh)
{
	...
	if (con->rma_mem_deregister && (req->pub.data_len > pscom_rendezvous_msg_len(0))) {
		con->rma_mem_deregister(con, rd);
	}
...
pscom_req_t *pscom_prepare_send_rendezvous_inline(pscom_req_t *user_req, pscom_msgtype_t msg_type)
{
	...
	/* rendezvous xheader = Rendezvous data, starting with pscom_rendezvous_msg_t rd->msg */
	...
	rndv_req->pub.xheader_len = pscom_rendezvous_xheader_len(len_arch, user_xheader_len);
...
void _pscom_post_rma_read(pscom_req_t *rma_read_req)
{
	...
	rma_read_req->pub.state = PSCOM_REQ_STATE_RMA_READ_REQUEST | PSCOM_REQ_STATE_POSTED;
	...
	if (con->rma_write && con->rma_mem_register) {
		...
		if(!con->rma_mem_register_check || (con->rma_mem_register_check && con->rma_mem_register_check(con, rd))) {
			...
			if(len_arch && con->rma_mem_deregister) {
				rma_read_req->rndv_data = pscom_malloc(sizeof(pscom_rendezvous_data_t));
				memcpy(rma_read_req->rndv_data, rd, sizeof(pscom_rendezvous_data_t)); // ToDo: _pscom_memcpy?
			}
		}
