                            ...
                            unsigned size)
{
    ...
    case PSCOM_INFO_ARCH_REQ: {
        ...
        DPRINT(D_PRECON_TRACE, "precon(%p):%s:%s %s\tarch_id:%u (%s)", pre, op,
               plugin_name, pscom_info_type_str(type), arch_req->arch_id,
    ...
    case PSCOM_INFO_VERSION: {
        ...
        DPRINT(D_PRECON_TRACE, "precon(%p):%s:%s %s\tver_from:%04x ver_to:%04x",
               pre, op, plugin_name, pscom_info_type_str(type),
...
static void pscom_precon_print_stat(pscom_precon_t *pre)
{
    ...
    DPRINT(D_PRECON_TRACE,
           ...
           pre->send_len, pre->recv_done ? "no" : "yes", pscom_precon_count,
           state);
}
...
static void tcp_configure(int fd)
{
    int ret;
    int val;
...
static int _pscom_tcp_connect(int nodeid, int portno, void *debug_id)
{
...
static void _plugin_connect_next(pscom_con_t *con, int first)
{
    ...
    do {
        pre->plugin      = first ? pscom_plugin_first()
                                 : pscom_plugin_next(pre->_plugin_cur);
    ...
    if (!pre->plugin) {
        ...
        // Try this plugin:
        pscom_precon_send(pre, PSCOM_INFO_ARCH_REQ, &pre->plugin->arch_id,
                          sizeof(pre->plugin->arch_id));
        pre->plugin->con_handshake(con, PSCOM_INFO_ARCH_REQ,
                                   &pre->plugin->arch_id,
                                   sizeof(pre->plugin->arch_id));
    }
}
...
void pscom_precon_send_PSCOM_INFO_CON_INFO(pscom_precon_t *pre, int type)
{
    ...
    DPRINT(D_PRECON_TRACE, "precon(%p): con:%s", pre,
           pscom_con_str(&pre->con->pub));
                                        ...
                                        void *data, unsigned size)
{
    ...
    case PSCOM_INFO_FD_ERROR:
        pscom_precon_abort_plugin(pre);
        err = data ? *(int *)data : 0;
        if (con && (!pre->back_connect ||   /* not a back connect */
    ...
    case PSCOM_INFO_BACK_CONNECT: {
        ...
        DPRINT(D_PRECON_TRACE, "precon(%p): recv backcon %.8s to %.8s", pre,
    ...
    case PSCOM_INFO_ARCH_STEP4:
        ...
        if (con) {
            if (pre->plugin) {
                ...
                if (type == PSCOM_INFO_ARCH_OK) { pscom_precon_close(pre); }
            } else {
...
static void pscom_precon_terminate_backconnect(pscom_precon_t *pre)
{
    ...
    DPRINT(D_DBG_V,
           ...
           "state:%8s",
           pre, pre->con, pscom_con_type_str(pre->con->pub.type),
...
static void pscom_precon_reconnect(pscom_precon_t *pre)
{
    ...
    if (pre->reconnect_cnt < pscom.env.retry) {
        pre->reconnect_cnt++;
        DPRINT(D_DBG, "precon(%p):pscom_precon_reconnect count %u", pre,
               pre->reconnect_cnt);
...
error:
    ...
    error_code = errno;
    pscom_precon_handle_receive(pre, PSCOM_INFO_FD_ERROR, &error_code,
                                sizeof(error_code));
    return;
}
...
static void pscom_precon_check_end(pscom_precon_t *pre)
{
    ...
    if ((pre->send_len == 0) && pre->recv_done) {
        if (!pre->back_connect) {
            ...
            if (pre->con) {
                pre->con->precon = NULL; // disallow precon usage in handshake
            }
...
static void pscom_precon_do_write(ufd_t *ufd, ufd_funcinfo_t *ufd_info)
{
    ...
    if (pre->send_len) {
        len = (int)send(pre->ufd_info.fd, pre->send, pre->send_len,
                        MSG_NOSIGNAL);
    } else {
        len = 0;
    }
    ...
    if (len >= 0) {
        ...
        if (pre->connect && retry_on_error(errno)) {
            ...
            pre = NULL; // pscom_precon_reconnect() might close pre. Don't use
                        // pre afterwards.
        } else {
            switch (errno) {
            case EAGAIN:
            ...
            default: {
                ...
                DPRINT(log_level, "precon(%p): write(%d, %p, %u) : %s", pre,
                       pre->ufd_info.fd, pre->send, pre->send_len,
                       strerror(errno));
                ufd_event_clr(&pscom.ufd, &pre->ufd_info, POLLOUT);
                close(pre->ufd_info.fd);
                pre->send_len = 0;
...
static void pscom_precon_do_read(ufd_t *ufd, ufd_funcinfo_t *ufd_info)
{
    ...
    if (pre->recv_len < header_size) {
        len = (int)read(fd, pre->recv + pre->recv_len,
                        header_size - pre->recv_len);
...
check_read_error:
    if (len == 0) {
         ...
         * pscom.env.precon_reconnect_timeout.*/
        pscom_precon_connect_terminate(pre);
    } else {
...
static int pscom_precon_do_read_poll(pscom_poll_t *poll)
{
    ...
    if (!pre->connect) {
        ...
        // finally.
        pscom_precon_terminate_backconnect(pre);
    } else if (now - pre->last_reconnect >
               pscom.env.precon_reconnect_timeout /* ms */ * 1000UL) {
        ...
        if (!pscom_precon_isconnected(pre)) {
            ...
            if (pre->stalled_cnt < pscom.env.precon_connect_stalled_max) {
                /* Wait */
                DPRINT(D_DBG, "precon(%p): connect(%s:%u) stalled %u/%u", pre,
                       pscom_inetstr(pre->nodeid), pre->portno,
                       pre->stalled_cnt, pscom.env.precon_connect_stalled_max);
            } else {
                DPRINT(D_ERR,
                       "precon(%p): connect(%s:%u) stalled - reconnecting", pre,
                       pscom_inetstr(pre->nodeid), pre->portno);
...
void pscom_con_accept(ufd_t *ufd, ufd_funcinfo_t *ufd_info)
{
    ...
    while (1) {
        ...
        assert(pre);
        DPRINT(D_PRECON_TRACE, "precon(%p): accept(%d,...) = %d", pre,
               listen_fd, fd);
