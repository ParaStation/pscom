...
static
void pscom_dump_ufd(FILE *out, ufd_t *ufd)
{
...
static
void pscom_dump_requests(FILE *out)
{
	struct list_head *pos;
...
void pscom_dump_con(FILE *out, pscom_con_t *con)
{
	...
	if (con->in.req) {
		fprintf(out, " ract:%p", &con->in.req->pub);
	}
	...
	if ((cnt = list_count(&con->recvq_rma))) {
		fprintf(out, " rrma:%5d", cnt);
	}
	fputs("\n", out);
}
...
static
void pscom_dump_connections(FILE *out, pscom_sock_t *sock)
{
	struct list_head *pos;
...
static
void pscom_dump_socket(FILE *out, pscom_sock_t *sock)
{
	fprintf(out, "  sock#%p listen:%6d demand:%4d(%4d)  src:%s anyrecv:%6u\n",
		&sock->pub, sock->pub.listen_portno,
		sock->listen.usercnt, sock->listen.activecnt,
...
static
void pscom_dump_sockets(FILE *out)
{
	struct list_head *pos;
...
{
	if (pscom.env.sigquit) {
		signal(SIGQUIT, pscom_sigquit);
	}
}
