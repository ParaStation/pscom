...
static unsigned int pscom_plugin_uprio(const pscom_con_type_t con_type)
{
    unsigned res;
    ...
    if ((con_type == PSCOM_CON_TYPE_EXTOLL) ||
        ...
        if (!env_extoll_initialized) {
            ...
            if (env_velo == PSCOM_ENV_UINT_AUTO) {
                // auto: enable "velo" only if "extoll" is disabled (or was
                // auto).
                env_velo = (env_extoll == 0) ? 1 : 0;
            }
            if (env_extoll && env_velo) {
                DPRINT(D_WARN, "'PSP_VELO' and 'PSP_EXTOLL' are mutually "
                               "exclusive! Disabling 'PSP_EXTOLL'");
                env_extoll = 0;
            }
        }
        if (con_type == PSCOM_CON_TYPE_EXTOLL) {
...
static void pscom_plugin_register(pscom_plugin_t *plugin, unsigned int user_prio)
{
    if (!user_prio) {
    ...
    if (tmpp) {
        DPRINT(D_DBG_V,
               "Arch id %d already registered (registered:%s, disabled:%s)",
               plugin->arch_id, tmpp->name, plugin->name);
        return; // disabled arch
    }


    DPRINT(D_DBG_V, "Register arch %s with priority %02d.%02d", plugin->name,
           plugin->user_prio, plugin->priority);
...
static pscom_plugin_t *load_plugin_lib(char *lib, const char *arch)
{
    void *libh;
    char *errstr;
    ...
    if (libh) {
        ...
        if (plugin) {
            if (plugin->version == PSCOM_PLUGIN_VERSION) {
                ...
                // Error
                DPRINT(D_ERR,
                       "Loading %s failed : Version mismatch (0x%04x != "
                       "expected 0x%04x)",
    ...
    errstr = dlerror();
    DPRINT(D_DBG_V, "Loading %s failed : %s", lib,
...
static const char *pscom_libdir_self(void)
{
    static const char *libdir;
    Dl_info info;

    if (libdir) {
        return libdir; // return cached one
    }
    ...
    if (dladdr(pscom_init, &info) && info.dli_fname) {
        ...
        if (dirend) {
            dirend[1] = 0; // like dirname(), but keep '/'
        }
        libdir = fname;
    } else {
...
static void pscom_plugin_load(const pscom_con_type_t con_type)
{
    ...
    const char *libdirs[] = {pscom.env.plugindir, // "" or environ,
                             pscom_libdir_self(), NULL};
    unsigned cnt          = 0;
    ...
    for (ld_p = libdirs; *ld_p; ld_p++) {
        ...
        snprintf(libpath, sizeof(libpath), "%slibpscom4%s.so", *ld_p, arch);
...
void pscom_plugins_init(void)
{
    ...
    pscom_plugin_register(&pscom_plugin_tcp,
                          pscom_plugin_uprio(PSCOM_CON_TYPE_TCP));
    pscom_plugin_register(&pscom_plugin_shm,
                          pscom_plugin_uprio(PSCOM_CON_TYPE_SHM));
#ifdef PSCOM_ALLIN_PSM2
    pscom_plugin_register(&pscom_plugin_psm,
                          pscom_plugin_uprio(PSCOM_CON_TYPE_PSM));
#endif
#ifdef PSCOM_ALLIN_OPENIB
    pscom_plugin_register(&pscom_plugin_openib,
                          pscom_plugin_uprio(PSCOM_CON_TYPE_OPENIB));
#endif
#ifdef PSCOM_ALLIN_UCP
    pscom_plugin_register(&pscom_plugin_ucp,
                          pscom_plugin_uprio(PSCOM_CON_TYPE_UCP));
#endif
#ifdef PSCOM_ALLIN_GATEWAY
    pscom_plugin_register(&pscom_plugin_gateway,
                          pscom_plugin_uprio(PSCOM_CON_TYPE_GW));
#endif
    ...
    pscom_con_type_t pls[] = {
...
#endif
        PSCOM_CON_TYPE_OFED,    PSCOM_CON_TYPE_MVAPI,  PSCOM_CON_TYPE_GM,
        PSCOM_CON_TYPE_ELAN,    PSCOM_CON_TYPE_EXTOLL, PSCOM_CON_TYPE_VELO,
        PSCOM_CON_TYPE_DAPL,    PSCOM_CON_TYPE_MXM,
        ...
        PSCOM_CON_TYPE_GW,
#endif
        PSCOM_CON_TYPE_PORTALS, PSCOM_CON_TYPE_NONE};
