...
pscom_env_table_entry_t pscom_env_table_shm[] = {
    {"RENDEZVOUS", PSCOM_ENV_UINT_INF_STR,
     "The rendezvous threshold for pscom4shm.", &pscom.env.rendezvous_size_shm,
...
static unsigned shm_direct   = SHM_DIRECT;
static unsigned shm_indirect = SHM_INDIRECT;
...
static int shm_poll_write_pending_io(pscom_poll_t *poll);


static int shm_init_direct(shm_conn_t *shm, int shmid, void *remote_base)
{
    if (shmid != -1) {
        // Use direct shm.
...
static int shm_initrecv(shm_conn_t *shm)
{
    int shmid;
    void *buf;
...
err_shmat:
    ...
    return -1;
err:
    DPRINT(D_ERR, "shmget(0, sizeof(shm_com_t), IPC_CREAT | 0777) : %s",
           strerror(errno));
    return -1;
}


static int shm_initsend(shm_conn_t *shm, shm_info_msg_t *msg)
{
    void *buf;
...
static inline int shm_cansend(shm_conn_t *shm)
{
...
*/
static size_t shm_iovsend(shm_conn_t *shm, struct iovec *iov, size_t len)
{
    int cur           = shm->send_cur;
    shm_buf_t *shmbuf = &shm->remote_com->buf[cur];
...
/* send iov.
   Call only if shm_cansend() == true (no check inside)!
   iov[0].iov_len must be smaller or equal SHM_BUFLEN - sizeof(struct
   shm_direct_header)! is_psshm_ptr(iov[1].iov_base) must be true.
*/
static shm_msg_t *shm_iovsend_direct(shm_conn_t *shm, struct iovec *iov)
{
    int cur           = shm->send_cur;
    shm_buf_t *shmbuf = &shm->remote_com->buf[cur];
...
static inline uint32_t shm_canrecv(shm_conn_t *shm)
{
...
*/
static inline void shm_recvstart(shm_conn_t *shm, char **buf, unsigned int *len)
{
    int cur           = shm->recv_cur;
    shm_buf_t *shmbuf = &shm->local_com->buf[cur];
...
*/
static inline void shm_recvstart_direct(shm_conn_t *shm, struct iovec iov[2])
{
    int cur           = shm->recv_cur;
    shm_buf_t *shmbuf = &shm->local_com->buf[cur];
...
static inline void shm_recvdone(shm_conn_t *shm)
{
    int cur           = shm->recv_cur;
    shm_buf_t *shmbuf = &shm->local_com->buf[cur];
...
static inline void shm_recvdone_direct(shm_conn_t *shm)
{
    int cur           = shm->recv_cur;
    shm_buf_t *shmbuf = &shm->local_com->buf[cur];
...
static int shm_do_read(pscom_poll_t *poll)
{
...
static int shm_poll_write_pending_io(pscom_poll_t *poll)
{
    ...
    while (((sp = shm->shm_pending)) &&
        ...
        // finish request
        if (sp->req) {
            pscom_write_pending_done(sp->con, sp->req); // direct send done
        }
        if (sp->data) {
            free(sp->data); // indirect send done
        }
 ...
 */
static void shm_pending_io_enq(pscom_con_t *con, shm_msg_t *msg,
                               pscom_req_t *req, void *data)
{
    ...
    if (!shm->shm_pending) {
        pscom_poll_start(&shm->poll_write_pending_io, shm_poll_write_pending_io,
                         &pscom.poll_write);
        shm->shm_pending = sp;
    } else {
        // Append at the end
...
static int shm_do_write(pscom_poll_t *poll)
{
    ...
    if (req && shm_cansend(&con->arch.shm)) {
        ...
        do_buffered_send:
            ...
            /* The shm_iovsend_direct is active as long as msg->msg_type ==
               SHM_MSGTYPE_DIRECT. We have to call pscom_write_pending_done(con,
               req) when we got the ack msg_type == SHM_MSGTYPE_DIRECT_DONE. */

            shm_pending_io_enq(con, msg, req, NULL);

            pscom.stat.shm_direct++;
        } else {
            /* Indirect send : Copy data iov[1] to a shared region and send a
             * reference to it. */
            /* Size is good for direct send, but the data is not inside the
             * shared mem region */
            ...
            if (!is_psshm_enabled() ||            // Direct shm is disabled.
                iov[1].iov_len <= shm_indirect) { // or (disabled or len to
                                                  // small) for indirect shm
                goto do_buffered_send;
            }

            data = malloc(iov[1].iov_len); // try to get a buffer inside the
                                           // shared mem region
            ...
            _pscom_memcpy_from_user(data, iov[1].iov_base, iov[1].iov_len);
            iov[1].iov_base = data;
...
static void shm_init_shm_conn(shm_conn_t *shm)
{
...
static void shm_cleanup_shm_conn(shm_conn_t *shm)
{
...
static void shm_close(pscom_con_t *con)
{
...
static void pscom_poll_read_start_shm(pscom_con_t *con)
{
    pscom_poll_read_start(con, shm_do_read);
}


static void pscom_poll_write_start_shm(pscom_con_t *con)
{
    pscom_poll_write_start(con, shm_do_write);
}


static void shm_init_con(pscom_con_t *con)
{
...
static int shm_is_local(pscom_con_t *con)
{
    return con->pub.remote_con_info.node_id == pscom_get_nodeid();
}

/****************************************************************/
static void pscom_shm_sock_init(pscom_sock_t *sock)
{

    /* register the environment configuration table */
    pscom_env_table_register_and_parse("pscom SHM", "SHM_", pscom_env_table_shm);
...
static void pscom_shm_info_msg(shm_conn_t *shm, shm_info_msg_t *msg)
{
...
static int pscom_shm_con_init(pscom_con_t *con)
{
                                ...
                                unsigned size)
{
    ...
    case PSCOM_INFO_EOF: shm_init_con(con); break;
    }
...
error_initsend:
    shm_cleanup_shm_conn(shm);
    pscom_precon_send_PSCOM_INFO_ARCH_NEXT(precon);
}
