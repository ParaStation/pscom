...
/* Used buffersize */
#define ELAN_BUFSIZE_PAYLOAD                                                   \
    (16 * 1024) /* must be < 65536, or change sizeof                           \
                   pselan_msgheader_t.payload */
...
// #define PSELAN_DATA_OFFSET(pos, pselanlen) ((pos) * ELAN_BUFSIZE +
// ELAN_BUFSIZE_PAYLOAD - (pselanlen))
...
#define pselan_dprint(level, fmt, arg...)                                      \
    do {                                                                       \
...
#define pselan_dprint_errno(level, _errno, fmt, arg...)                        \
    do {                                                                       \
        if ((level) <= pselan_debug) {                                         \
            pselan_dprint(level, fmt " : %s", ##arg, strerror(_errno));        \
        }                                                                      \
    } while (0);
...
static void pselan_init_buffers_local(pselan_con_info_t *ci)
{
    int i;
...
static void pselan_queue_event(pselan_con_info_t *ci, ELAN_EVENT *event)
{
    if (ci->event) {
...
static void pselan_flush_event(pselan_con_info_t *ci)
{
    // Flush events:
    if (ci->event && elan_poll(ci->event, 0)) { ci->event = NULL; }
}


static inline void pselan_flush_sendbuf(pselan_con_info_t *ci,
                                        char *lmem /* ci->send_bufs.lmr_mem */,
                                        char *rmem, unsigned size)
{
    ELAN_EVENT *event;

    event = elan_put(pselan_base_state, lmem, rmem, size, ci->remote_destvp);
...
static void pselan_get_fresh_tokens(pselan_con_info_t *ci);


/* returnvalue like write(), except on error errno is negative return */
static int _pselan_sendv(pselan_con_info_t *ci, struct iovec *iov, int size,
                         unsigned int magic)
{
    int len;
    int pselanlen;
    ...
    */
    ...
    len       = (size <= (int)ELAN_BUFSIZE_PAYLOAD) ? size
                                                    : (int)ELAN_BUFSIZE_PAYLOAD;
...
static void pselan_get_fresh_tokens(pselan_con_info_t *ci)
{
    ...
    if ((magic == PSELAN_MAGIC_IO) && (msg->tail.payload == 0)) {
        // Fresh tokens
        msg->tail.magic = PSELAN_MAGIC_UNUSED;
        ci->n_send_toks += msg->tail.token;
        ci->n_recv_toks++;
